...
def converter_data(valor):
    ...
    if isinstance(valor, (int, float)):
        try:
            return (datetime(1899, 12, 30) + pd.Timedelta(days=valor)).strftime(
                "%Y-%m-%d"
            )
        except:
            return None
...
def transformar_arquivo(input_path, output_path):
    ...
    try:
        df = pd.read_excel(
            input_path, sheet_name="Bombeiro_Municipal", dtype={"CNH": str, "RG": str}
        )
    ...
    df["nome_de_guerra"] = (
        df["nome"].str.split().str[0]
    )  # Primeiro nome como nome de guerra
...
if __name__ == "__main__":
    ...
    input_file = input(
        "Digite o nome do arquivo Excel de entrada (ex: Bombeiro Municipal.xlsx): "
    )
    output_file = input(
        "Digite o nome do arquivo CSV de saída (ex: bombeiros_formatado.csv): "
    )
