...
class ChamadoForm(forms.ModelForm):
    ...
    categoria_principal = forms.ChoiceField(
        choices=[('', 'Selecione uma categoria')] + list(Categoria.TIPO_PROBLEMA_CHOICES),
    ...
    class Meta:
        model = Chamado
        fields = [
            'solicitante_cpf', 'assunto', 'descricao'
        ]
        widgets = {
            'solicitante_cpf': forms.TextInput(attrs={
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.is_bound and self.data.get('categoria_principal'):
            ...
            sub_choices = [(sub.subcategoria, sub.get_subcategoria_display()) for sub in sub_categorias]
            self.fields['subcategoria'].choices = [('', 'Selecione uma subcategoria')] + sub_choices
    ...
    def clean(self):
        ...
        if cat_principal and sub_cat:
            try:
                categoria_obj = Categoria.objects.get(categoria=cat_principal, subcategoria=sub_cat)
                self.cleaned_data['categoria'] = categoria_obj
            except Categoria.DoesNotExist:
                raise forms.ValidationError("A combinação de categoria e subcategoria selecionada é inválida.")
        else:
            if not self.errors:
                raise forms.ValidationError("Categoria e Subcategoria são obrigatórias.")
