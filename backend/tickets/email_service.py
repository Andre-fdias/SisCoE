...
def enviar_email_chamado_aberto(chamado):
    ...
    try:
        ...
        html_content = render_to_string('tickets/email/novo_chamado.html', {
            'chamado': chamado,
        ...
        if result:
            logger.info(f"✅ Email de abertura enviado para {chamado.solicitante_email}")
        else:
            logger.error(f"❌ Falha ao enviar email de abertura para {chamado.solicitante_email}")
...
def enviar_email_mudanca_status(chamado, old_status, new_status, comentario=None):
    ...
    try:
        ...
        html_content = render_to_string('tickets/email/mudanca_status.html', {
            'chamado': chamado,
            'old_status': old_status,
            'new_status': new_status,
            'old_status_display': chamado.get_status_display_color().replace(old_status, '').strip(),
            'new_status_display': chamado.get_status_display_color().replace(new_status, '').strip(),
        ...
        if result:
            logger.info(f"✅ Email de mudança de status enviado para {chamado.solicitante_email}")
        else:
            logger.error(f"❌ Falha ao enviar email de status para {chamado.solicitante_email}")
            
        return result
        
    except Exception as e:
        logger.error(f"❌ Erro ao enviar email de mudança de status: {str(e)}", exc_info=True)
        return False
...
def enviar_email_comentario_adicionado(chamado, comentario):
    ...
    try:
        ...
        html_content = render_to_string('tickets/email/novo_comentario.html', {
            'chamado': chamado,
            'comentario': comentario,
            'comentarios': comentarios,
        })
        ...
        if result:
            logger.info(f"✅ Email de comentário enviado para {chamado.solicitante_email}")
        else:
            logger.error(f"❌ Falha ao enviar email de comentário para {chamado.solicitante_email}")
