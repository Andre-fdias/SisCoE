...
class Medalha(models.Model):
    ...
    cadastro = models.ForeignKey(
        Cadastro, related_name="medalhas_concedidas", on_delete=models.DO_NOTHING
    )
    honraria = models.CharField(max_length=255, choices=HONRARIA_CHOICES)
    bol_g_pm_lp = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="BOL GPm LP"
    )
    data_publicacao_lp = models.DateField(
        null=True, blank=True, verbose_name="Data Publicação LP"
    )
    ...
    def get_search_result(self):
        return {
            ...
            "fields": {
                ...
                "Publicação": (
                    self.data_publicacao_lp.strftime("%d/%m/%Y")
                    if self.data_publicacao_lp
                    else "-"
                ),
            },
        }
...
class Curso(models.Model):
    ...
    cadastro = models.ForeignKey(
        Cadastro, on_delete=models.CASCADE, related_name="cursos"
    )
    curso = models.CharField(max_length=255, choices=CURSOS_CHOICES)

    data_publicacao = models.DateField(
        verbose_name="Data de Publicação"
    )  # Campo único para data
