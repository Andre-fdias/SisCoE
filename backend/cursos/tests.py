...
class MedalhaModelTest(TestCase):
    ...
    def test_get_search_result_medalha(self):
        ...
        self.assertEqual(
            search_result["title"], f"{medalha.honraria} - {medalha.cadastro.nome}"
        )
...
class CursoModelTest(TestCase):
    ...
    def test_get_search_result_curso(self):
        ...
        self.assertEqual(
            search_result["title"], f"{curso.curso} - {curso.cadastro.nome}"
        )
        self.assertEqual(search_result["fields"]["Curso"], curso.curso)
        self.assertEqual(
            search_result["fields"]["BOL Publicação"], curso.bol_publicacao
        )
        self.assertEqual(search_result["fields"]["Publicação"], "15/01/2023")
    ...
    def test_curso_tags_mapping(self):
        ...
        self.assertEqual(
            Curso.CURSOS_TAGS["Gestão pela Qualidade_Oficial"], "Administrativo"
        )
        self.assertEqual(
            Curso.CURSOS_TAGS["Atendimento Pré-Hospitalar Tático"], "Operacional"
        )
