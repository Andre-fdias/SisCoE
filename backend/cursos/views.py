...
from .resources import (
    MedalhaResource,
    CursoResource,
)  # Certifique-se de ter esses resources
...
def get_cadastro_do_usuario(user):
    ...
    try:
        ...
        logger.error(
            f"Erro inesperado ao obter Cadastro para o usuário {user.username}: {str(e)}",
            exc_info=True,
        )
        return None
        ...
        return JsonResponse(
            {
                ...
                "data_publicacao_lp": (
                    medalha.data_publicacao_lp.strftime("%Y-%m-%d")
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if data_publicacao_lp:
                    medalha.data_publicacao_lp = datetime.strptime(
                        data_publicacao_lp, "%Y-%m-%d"
                    ).date()
...
def medalha_delete(request, pk):
    ...
    try:
        medalha.delete()
        messages.success(
            request, "Medalha excluída com sucesso!", extra_tags="success_message"
        )  # Adicione extra_tags
    except Exception as e:
        messages.error(
            request, f"Erro ao excluir medalha: {str(e)}", extra_tags="error_message"
        )  # Adicione extra_tags
        ...
        try:
            with transaction.atomic():
                ...
                for i in range(num_medalhas):
                    ...
                    if not honraria:
                        raise ValueError(
                            f"Honraria é obrigatória para a medalha #{i + 1}"
                        )

                    bol_g_pm_lp = (
                        request.POST.get(f"bol_g_pm_lp_{i}", "").strip() or None
                    )
                    data_publicacao_lp = (
                        request.POST.get(f"data_publicacao_lp_{i}") or None
                    )
                    observacoes = (
                        request.POST.get(f"observacoes_{i}", "").strip() or None
                    )
                    ...
                    if data_publicacao_lp:
                        try:
                            ...
                            raise ValueError(
                                f"Data inválida na medalha #{i + 1}. Use AAAA-MM-DD."
                            )
                ...
                messages.success(
                    request,
                    f"{num_medalhas} medalha(s) cadastrada(s)!",
            ...
            messages.error(
                request, "Militar não encontrado.", extra_tags="error_message"
            )  # Adicione extra_tags
        except ValueError as e:
            messages.error(
                request, str(e), extra_tags="error_message"
            )  # Adicione extra_tags
        except Exception as e:
            messages.error(
                request, f"Erro: {str(e)}", extra_tags="error_message"
            )  # Adicione extra_tags
...
def buscar_militar_medalha(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            context = {
                "cadastro": cadastro,
                "detalhes": {"sgb": sgb, "posto_secao": posto_secao},
                "promocao": {"posto_grad": posto_grad},
                "imagem": imagem,
                "honraria_choices": Medalha.HONRARIA_CHOICES,
                "found_re": re,
            }
            ...
            return render(
                request,
                template_name,
                {"honraria_choices": Medalha.HONRARIA_CHOICES, "searched_re": re},
            )
...
def import_medalhas_csv(request):
    if request.method == "POST":
        ...
        if "csv_file" not in request.FILES:
            messages.error(
                request,
                "Nenhum arquivo CSV foi selecionado para importação.",
        ...
        if not csv_file.name.endswith(".csv"):
            messages.error(
                request,
                "Formato de arquivo inválido. Por favor, envie um arquivo CSV.",
                extra_tags="error_message",
            )
            return redirect("cursos:medalha_list")
        ...
        try:
            ...
            messages.error(
                request,
                "Não foi possível decodificar o arquivo CSV. Verifique a codificação (tente UTF-8).",
                extra_tags="error_message",
            )
            return redirect("cursos:medalha_list")
        except Exception as e:
            messages.error(
                request,
                f"Erro ao ler o arquivo CSV: {str(e)}",
                extra_tags="error_message",
            )
            return redirect("cursos:medalha_list")
        ...
        try:
            with transaction.atomic():  # Garante atomicidade da transação
                ...
                result = medalha_resource.import_data(
                    data_set, dry_run=True, raise_errors=True, collect_failed_rows=True
                )

                if not result.has_errors() and not result.has_validation_errors():
                    # Se não houver erros, executa a importação real
                    medalha_resource.import_data(
                        data_set,
                        dry_run=False,
                        raise_errors=True,
                    ...
                    messages.success(
                        request,
                        "Medalhas importadas com sucesso!",
                        extra_tags="success_message",
                    )
                else:
                        ...
                        for row_errors in result.validation_errors:
                            for field_error in row_errors.field_errors:
                                error_messages.append(
                                    f"Linha {row_errors.row_num}: Campo '{field_error.field}': {field_error.errors[0]}"
                                )
                    if result.has_errors():
                        for error in result.errors:
                            error_messages.append(
                                f"Erro na linha {error.line}: {error.error_message}"
                            )

                    messages.error(
                        request,
                        "Erro(s) na importação: <br>" + "<br>".join(error_messages),
                        extra_tags="error_message",
                    )
            ...
            messages.error(
                request,
                f"Erro de validação durante a importação: {str(e)}",
                extra_tags="error_message",
            )
        except Exception as e:
            messages.error(
...
def curso_create(request):
    ...
    """
    context = {"title": "Cadastrar Curso", "curso_choices": Curso.CURSOS_CHOICES}
    ...
    if request.method == "POST":
        ...
        if not cadastro_id:
            messages.error(
                request,
                "É necessário buscar e selecionar um militar antes de cadastrar cursos.",
            )
        ...
        try:
            ...
            if not request.user.is_superuser and request.user.permissoes not in [
                "admin",
                "gestor",
            ]:
                if not (
                    hasattr(request.user, "cadastro")
                    and request.user.cadastro
                    and hasattr(cadastro, "detalhes_situacao")
                    and cadastro.detalhes_situacao.filter(
                        sgb=request.user.get_user_sgb()
                    ).exists()
                ):
                    messages.error(
                        request,
                        "Você não tem permissão para cadastrar cursos para este militar.",
                        extra_tags="error_message",
                    )
                    return redirect("cursos:buscar_militar_curso")
            ...
            with transaction.atomic():
                for i in range(num_cursos):
                    ...
                    if not curso_tipo or not data_publicacao_str or not bol_publicacao:
                        messages.warning(
                            request,
                            f"Ignorando curso {i+1}: Campos obrigatórios não preenchidos.",
                        )
                        continue
                    ...
                    try:
                        ...
                        if data_publicacao > date.today():
                            messages.warning(
                                request,
                                f"Ignorando curso #{i + 1}: Data de publicação não pode ser futura.",
                            )
                            continue
                        ...
                        messages.warning(
                            request,
                            f"Ignorando curso {i+1}: Formato de data inválido. Use AAAA-MM-DD.",
                        )
                        continue
            ...
            if cursos_salvos_count > 0:
                messages.success(
                    request,
                    f"{cursos_salvos_count} curso(s) cadastrado(s) com sucesso para o RE {re}!",
                )
                return redirect("cursos:curso_list")
            else:
                messages.info(
                    request,
                    "Nenhum curso foi cadastrado. Verifique os dados e tente novamente.",
                )
                context.update(
                    {
                        "found_militar": True,
...
from django.contrib.auth.decorators import login_required
from django.views.decorators.http import require_http_methods
from .models import Cadastro  # Make sure to import Curso
...
def curso_edit(request, pk):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return JsonResponse(
                {"success": True, "message": "Curso atualizado com sucesso!"}
            )
...
def curso_delete(request, pk):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"Erro ao excluir o curso: {str(e)}"},
            status=400,
        )
...
def buscar_militar_curso(request):
    ...
    print("--- FIM DEBUG VIEWS.PY - buscar_militar_curso ---\n")
    ...
    if request.method == "POST":
        ...
        try:
            ...
            imagem_obj = (
                Imagem.objects.filter(cadastro=cadastro).order_by("-id").first()
            )
            ...
            context.update(
                {
                    ...
                    "promocao": {
                        "posto_grad": (
                            promocao_obj.posto_grad if promocao_obj else "Não informado"
                        )
                    },
                    "imagem": imagem_obj,
                    "found_re": re,
            ...
            if not detalhes_obj:
                messages.info(
                    request, "Atenção: Dados de SGB e Posto/Seção não encontrados."
                )
            if not promocao_obj:
                messages.info(
                    request, "Atenção: Dados de Posto/Graduação não encontrados."
                )
            if not imagem_obj:
...
def export_cursos_csv(request):
    ...
    queryset = Curso.objects.all().select_related(
        "cadastro"
    )  # Inclua select_related se precisar de dados do Cadastro
...
def import_cursos_csv(request):
    if request.method == "POST":
        if "csv_file" not in request.FILES:
            messages.error(
                request,
                "Nenhum arquivo CSV foi selecionado para importação de cursos.",
                extra_tags="error_message",
            )
            return redirect("cursos:curso_list")
        ...
        if not csv_file.name.endswith(".csv"):
            messages.error(
                request,
                "Formato de arquivo inválido. Por favor, envie um arquivo CSV para cursos.",
                extra_tags="error_message",
            )
            return redirect("cursos:curso_list")
        ...
        try:
            ...
            messages.error(
                request,
                "Não foi possível decodificar o arquivo CSV de cursos. Verifique a codificação (tente UTF-8).",
                extra_tags="error_message",
            )
            return redirect("cursos:curso_list")
        except Exception as e:
            messages.error(
                request,
                f"Erro ao ler o arquivo CSV de cursos: {str(e)}",
                extra_tags="error_message",
            )
            return redirect("cursos:curso_list")
        ...
        try:
            with transaction.atomic():
                result = curso_resource.import_data(
                    data_set, dry_run=True, raise_errors=True, collect_failed_rows=True
                )

                if not result.has_errors() and not result.has_validation_errors():
                    curso_resource.import_data(
                        data_set,
                        dry_run=False,
                        raise_errors=True,
                        collect_failed_rows=True,
                    ...
                    messages.error(
                        request,
                        "Erro(s) na importação de cursos: <br>"
            ...
            messages.error(
                request,
                f"Erro de validação durante a importação de cursos: {str(e)}",
                extra_tags="error_message",
            )
        except Exception as e:
            messages.error(
                request,
                f"Erro inesperado durante a importação de cursos: {str(e)}",
                extra_tags="error_message",
            )
...
# Importações padrão do Django
from django.contrib.auth.decorators import login_required
from django.http import Http404  # Adicionado HttpResponse, Http404
from django.views.decorators.http import require_http_methods
...
def user_curso_list(request):
    if not hasattr(request.user, "cadastro") or not request.user.cadastro:
        messages.error(
            request,
            "Seu perfil não está vinculado a um cadastro militar.",
            extra_tags="error_message",
        )
...
def user_curso_create(request):
    ...
    try:
        ...
        logger.error(
            f"Erro inesperado ao carregar cadastro para criação de cursos: {str(e)}",
            exc_info=True,
        )
        if request.method == "GET":
            messages.error(
                request,
                f"Erro inesperado ao carregar dados do perfil: {str(e)}",
                extra_tags="error_message",
            )
            return redirect("core:index")
        return JsonResponse(
            ...
            for i in range(num_cursos):
                ...
                try:
                    ...
                    if data_publicacao > timezone.now().date():
                        errors.append(
                            f"Curso #{i+1}: Data de publicação não pode ser futura."
                        )
                        continue

                except ValueError:
                    errors.append(
                        f"Curso #{i+1}: Formato de data inválido. Use AAAA-MM-DD."
                    )
                    continue
                ...
                try:
                    ...
                    logger.error(
                        f"Erro ao salvar curso #{i+1}: {str(e)}", exc_info=True
                    )
...
def user_curso_edit(request, pk):
    ...
    try:
        ...
        curso = get_object_or_404(
            Curso, pk=pk, cadastro__user_account=request.user
        )  # Ajuste para user_account
        ...
        logger.error(
            f"Erro inesperado ao atualizar curso {pk}: {str(e)}", exc_info=True
        )
        return JsonResponse(
            {
                "success": False,
...
def user_curso_delete(request, pk):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "Erro interno do servidor ao excluir o curso."},
            status=500,
        )
...
def user_medalha_list(request):
    try:
        ...
        logger.error(
            f"Erro inesperado ao carregar lista de medalhas do usuário: {str(e)}",
            exc_info=True,
        )
        messages.error(
            request,
            f"Erro inesperado ao carregar suas medalhas: {str(e)}",
            extra_tags="error_message",
        )
        return redirect("core:index")
...
def user_medalha_create(request):
    ...
    try:
        ...
        logger.error(
            f"Erro inesperado ao carregar cadastro para criação de medalhas: {str(e)}",
            exc_info=True,
        )
        if request.method == "GET":
            messages.error(
    ...
    if request.method == "POST":
        ...
        medalhas_salvas_count = (
            0  # Corrigido: 'medalhas_salvos_count' para 'medalhas_salvas_count'
        )
        errors = []
        ...
        with transaction.atomic():  # Garante que todas as medalhas são salvas ou nenhuma
            for i in range(num_medalhas):
                ...
                if not data_publicacao_lp_str:
                    errors.append(
                        f"Medalha #{i+1}: Data de Publicação LP é obrigatória."
                    )
                    continue
                ...
                if data_publicacao_lp_str:
                    try:
                        data_publicacao_lp = datetime.strptime(
                            data_publicacao_lp_str, "%Y-%m-%d"
                        ).date()
                    except ValueError:
                        errors.append(
                            f"Medalha #{i+1}: Formato de data inválido. Use AAAA-MM-DD."
                        )
                        continue
                ...
                try:
                    ...
                    logger.error(
                        f"Erro ao salvar medalha #{i+1}: {str(e)}", exc_info=True
                    )
        ...
        if errors:
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": f"{medalhas_salvas_count} medalha(s) cadastrada(s) com sucesso!",
                }
            )
        else:
            return JsonResponse(
                {
                    "success": False,
                    "error": "Nenhuma medalha foi cadastrada. Verifique os dados e tente novamente.",
                },
                status=400,
            )
...
def user_medalha_edit(request, pk):
    ...
    try:
        ...
        medalha = get_object_or_404(
            Medalha, pk=pk, cadastro__user_account=request.user
        )  # Ajuste para user_account
        ...
        if request.method == "GET":
            ...
            try:
                with transaction.atomic():
                    honraria = request.POST.get("honraria")
                    bol_g_pm_lp = request.POST.get("bol_g_pm_lp", "").strip() or None
                    data_publicacao_lp_str = (
                        request.POST.get("data_publicacao_lp") or None
                    )
                    observacoes = request.POST.get("observacoes", "").strip() or None
                    ...
                    if data_publicacao_lp_str:
                        try:
                            ...
                            raise ValueError(
                                "Formato de data de publicação inválido. Use AAAA-MM-DD."
                            )
                ...
                return JsonResponse(
                    {"success": True, "message": "Medalha atualizada com sucesso!"}
                )
            except ValueError as e:
                logger.error(
                    f"Erro de validação ao atualizar medalha {pk}: {str(e)}",
                    exc_info=True,
                )
                return JsonResponse({"success": False, "error": str(e)}, status=400)
            except Exception as e:
                logger.error(
                    f"Erro inesperado ao atualizar medalha {pk}: {str(e)}",
                    exc_info=True,
                )
                return JsonResponse(
                    {
                        "success": False,
                        "error": "Erro interno do servidor ao atualizar a medalha.",
                    },
                    status=500,
                )
    except Http404:
...
def user_medalha_delete(request, pk):
    ...
    try:
        ...
        # Removido messages.success para evitar conflito com JsonResponse
        return JsonResponse(
            {"success": True, "message": "Medalha excluída com sucesso!"}
        )
    except Http404:
        return JsonResponse(
            {
