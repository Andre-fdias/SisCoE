...
class MedalhaResource(resources.ModelResource):
    ...
    class Meta:
        ...
        exclude = ('usuario_cadastro', 'data_cadastro', 'usuario_alteracao', 'data_alteracao', 'cadastro') 
        # O campo 'cadastro' original é excluído pois estamos usando 'cadastro_re'
        
        # Campos necessários para importação
        import_id_fields = ('id',) # Use 'id' para atualizar registros existentes, se aplicável
    ...
    def before_import_row(self, row, **kwargs):
        ...
        if re_militar:
            try:
                ...
                raise ValueError(f"Militar com RE '{re_militar}' não encontrado na linha: {row}")
    ...
    def get_export_headers(self, selected_fields=None):
        ...
        for field_name in self.fields.keys():
            if field_name in self.Meta.fields: # Apenas campos que estão na lista de exportação
