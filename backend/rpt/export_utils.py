...
def export_to_pdf_rpt(request, data):
    ...
    styles.add(ParagraphStyle( # Estilo para o total por seção
            name='TotalPosto',
            fontSize=8,
    ...
    try:
        ...
        ], colWidths=[doc.width*0.15, doc.width*0.7, doc.width*0.15]) # Usa doc.width para responsividade à margem
    ...
    for item in data:
        ...
        sgb_display = POSTO_SECAO_MAP.get(item.sgb_destino, item.sgb_destino) # Nome para exibir
    ...
    table_header = [
        'Posto/Grad', 'RE', 'Nome Guerra', #'Nome de Guerra',
    ...
    table_style = TableStyle([
        ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#E0E0E0')), # Cor cabeçalho
        ...
        for posto_display, items in postos.items():
            ...
            for item in items:
                ...
                table_data.append([
                    ...
                    str((datetime.now().date() - item.data_pedido).days) # Calcula dias
                ])
                count_posto += 1

            # Cria a tabela para este posto/seção
            table = Table(table_data,
                          colWidths=[25*mm, 15*mm, 40*mm, 20*mm, 55*mm, 20*mm, 10*mm], # Ajuste larguras
                          style=table_style)

            posto_elements.append(table)
            posto_elements.append(Paragraph(f"Total {posto_display}: {count_posto}", styles['TotalPosto']))
    ...
    def add_watermark(canvas, doc):
        ...
        cpf = getattr(getattr(user, 'profile', None), 'cpf', '') # Obtem CPF ou string vazia
        ...
        for x in range(-int(A4[1]), int(A4[0] * 1.5), 250): # Ajusta range e espaçamento X
            for y in range(-int(A4[0]), int(A4[1] * 1.5), 150): # Ajusta range e espaçamento Y
                canvas.saveState()
...
def export_to_excel_rpt(request, data):
    ...
    headers = [
        ...
        'SGB Origem', 'Posto Origem',
        'Data Pedido', 'Dias Espera' # Nome da coluna ajustado
    ]
    ws.append(headers)
    ...
    for row in ws.iter_rows(min_row=6, max_col=7, max_row=ws.max_row): # Começa após cabeçalhos
...
def export_to_csv_rpt(request, data):
    ...
    writer = csv.writer(wrapper, delimiter=';', quoting=csv.QUOTE_MINIMAL) # Usar QUOTE_MINIMAL ou ALL
