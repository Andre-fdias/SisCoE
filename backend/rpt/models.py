...
class Cadastro_rpt(models.Model):
    ...
    doc_movimentacao = models.CharField(
        max_length=50, blank=True, null=True
    )  # Opcional
    alteracao = models.CharField(
        max_length=50, choices=alteracao_choices, blank=True, null=True
    )  # Opcional
    cadastro = models.ForeignKey(
        Cadastro, related_name="cadastro_rpt", on_delete=models.DO_NOTHING
    )
    ...
    def pedido_status(self):
        ...
        if dias < 30:
            return mark_safe(
                '<span class="bg-green-500 text-white px-2 py-1 rounded">- de 1 mês</span>'
            )
        if dias < 180:
            return mark_safe(
                '<span class="bg-yellow-500 text-white px-2 py-1 rounded">- de 06 meses</span>'
            )
        if dias < 365:
            return mark_safe(
                '<span class="bg-yellow-500 text-white px-2 py-1 rounded">- de 1 ano</span>'
            )
        if dias < 730:
            return mark_safe(
                '<span class="bg-red-500 text-white px-2 py-1 rounded">- de 2 anos</span>'
            )
        if dias > 730:
            return mark_safe(
                '<span class="bg-gray-800 text-white px-2 py-1 rounded">+ de 2 anos</span>'
            )
    ...
    def status_badge(self):
        if self.status == "Aguardando":
            return mark_safe(
                '<span class="bg-green-500 text-white px-2 py-1 rounded">Aguardando</span>'
            )
        if self.status == "Mov. serviço":
            return mark_safe(
                '<span class="bg-gray-500 text-white px-2 py-1 rounded">Mov. Serviço</span>'
            )
        if self.status == "Mov. própria":
            return mark_safe(
                '<span class="bg-blue-500 text-white px-2 py-1 rounded">Mov. Própria</span>'
            )
        if self.status == "Exclusão a pedido":
            return mark_safe(
                '<span class="bg-indigo-500 text-white px-2 py-1 rounded">Exclusão a Pedido</span>'
            )
        if self.status == "Tranferido de Unidade":
            return mark_safe(
                '<span class="bg-yellow-500 text-white px-2 py-1 rounded">Transferido de Unidade</span>'
            )
        if self.status == "Excluído Alt. quadro":
            return mark_safe(
                '<span class="bg-gray-800 text-white px-2 py-1 rounded">Excluído Alt. Quadro</span>'
            )
...
class HistoricoRpt(models.Model):
    ...
    cadastro = models.ForeignKey(
        Cadastro_rpt, related_name="Historicorpt", on_delete=models.CASCADE
    )
    usuario_alteracao = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True
    )
