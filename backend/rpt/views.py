...
def cadastrar_rpt(request):
    if request.method == "GET":
        context = {
            ...
            "posto_secao_choices": Cadastro_rpt._meta.get_field(
                "posto_secao_destino"
            ).choices,
        ...
        if not cadastro_id:
            messages.add_message(
                request,
                constants.ERROR,
                "Cadastro não encontrado. Por favor, pesquise um RE válido.",
            )
        ...
        cadastro_existente = Cadastro_rpt.objects.filter(
            cadastro=cadastro, status="Aguardando"
        ).exists()
        if cadastro_existente:
            messages.add_message(
                request, constants.ERROR, "Militar ja possui cadasto ativo no Rpt."
            )
...
def ver_rpt(request, id):
    ...
    count_subquery = (
        Cadastro_rpt.objects.filter(posto_secao_destino=OuterRef("posto_secao_destino"))
        .values("posto_secao_destino")
    ...
    inscritos_secao = (
        ...
        .order_by("posicao")
    )  # Ordenar pela posição calculada
    posicao_real = None
    for idx, inscrito in enumerate(
        inscritos_secao.filter(status="Aguardando"), start=1
    ):
...
def editar_rpt(request, id):
    ...
    if request.method == "GET":
        data = {
            ...
            "data_movimentacao": (
                str(cadastro_rpt.data_movimentacao)
                if cadastro_rpt.data_movimentacao
                else None
            ),
        ...
        cadastro_rpt.data_alteracao = (
            request.POST.get("data_alteracao") or None
        )  # Garante que None seja atribuído se vazio
        ...
        try:
            ...
            logger.error(
                f"Erro de validação ao salvar Cadastro_rpt: {e}", exc_info=True
            )
            return JsonResponse({"error": f"Erro de validação: {e}"}, status=400)
        except IntegrityError as e:
            logger.error(
                f"Erro de integridade ao salvar Cadastro_rpt: {e}", exc_info=True
            )
...
def excluir_rpt(request, id):
    ...
    if request.method == "POST":
        try:
            ...
            if not password:
                messages.add_message(
                    request, messages.ERROR, "Senha não fornecida. Operação cancelada."
                )
                return redirect(error_redirect_url)  # Redireciona de volta

            if not check_password(password, current_user.password):
                messages.add_message(
                    request, messages.ERROR, "Senha incorreta! Operação cancelada."
                )
                return redirect(error_redirect_url)  # Redireciona de volta
            ...
            messages.add_message(
                request, messages.SUCCESS, "Registro RPT excluído com sucesso."
            )
            return redirect("rpt:listar_rpt")  # Redireciona para a lista após sucesso

        except Exception as e:
            messages.add_message(
                request, messages.ERROR, f"Erro ao excluir o registro RPT: {str(e)}"
            )
...
def buscar_militar_rpt(request):
    ...
    if request.method == "POST":
        ...
        if not re:
            messages.add_message(
                request, constants.WARNING, "Por favor, informe o RE para buscar."
            )
        ...
        try:
            ...
            if not detalhes:
                messages.info(
                    request,
                    "Informação: Detalhes da situação atual não encontrados para este RE.",
                )
            if not promocao:
                messages.info(
                    request,
                    "Informação: Dados da última promoção não encontrados para este RE.",
                )
            if not imagem:
                messages.info(
                    request, "Informação: Imagem não encontrada para este RE."
                )
            ...
            messages.add_message(
                request, constants.ERROR, f'Cadastro com RE "{re}" não encontrado.'
            )
            ...
            messages.add_message(
                request,
                constants.ERROR,
                f'Múltiplos cadastros encontrados com RE "{re}". Verifique a base de dados.',
            )
            return render(
                request,
                cadastrar_template,
            ...
            messages.add_message(
                request,
                constants.ERROR,
                f"Ocorreu um erro inesperado ao buscar o militar: {str(e)}",
            )
            # Renderiza o form de cadastro vazio novamente
            return render(
                request,
...
def historico_rpt(request, id):
    ...
    """
    cadastro_rpt = get_object_or_404(
        Cadastro_rpt.objects.select_related("cadastro"), id=id
    )
    # Busca o histórico, otimizando a busca pelo usuário que fez a alteração
    historico_rpt_list = (
        HistoricoRpt.objects.filter(cadastro=cadastro_rpt)
        .select_related("usuario_alteracao__profile")
...
permission_required = user_passes_test(
    lambda u: u.is_superuser or u.has_perm("rpt.add_cadastro_rpt")
)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if extensao == "csv":
                ...
                )  # na_values=[] similar a keep_default_na=False
            else:
                messages.error(
                    request,
                    f'Formato de arquivo "{extensao}" não suportado. Use CSV (separado por ponto e vírgula) ou Excel (XLS, XLSX).',
                )
                return redirect(importar_url_name)
            ...
            messages.error(
                request,
                f"Erro ao ler o arquivo: {e}. Verifique se o arquivo está no formato correto, não corrompido, e se a codificação/separador (para CSV) estão corretos.",
            )
            return redirect(importar_url_name)
        ...
        if not colunas_obrigatorias.issubset(colunas_arquivo):
            ...
            messages.error(
                request,
                f'Colunas obrigatórias faltando no arquivo: {", ".join(sorted(list(missing)))}',
            )
            return redirect(importar_url_name)
        ...
        def converter_data(valor_str, field_name):
            ...
            if not valor_str or valor_str.lower() in [
                "na",
                "n/a",
                "nan",
                "none",
            ...
            raise ValueError(
                f'Formato de data inválido ou não reconhecido ("{valor_str}") para o campo "{field_name}"'
            )
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                if row.isnull().all() or all(
                    str(v).strip().lower() in ["", "na", "n/a", "nan", "none", "#n/d"]
                    for v in row.values
                ):
                    continue
                ...
                if not cadastro_re:
                    raise ValueError('Coluna "cadastro_re" não pode estar vazia.')

                # Busca o Cadastro associado
                try:
                    # select_related(None) força a não buscar relações automaticamente aqui
                    cadastro_obj = Cadastro.objects.select_related(None).get(
                        re=cadastro_re
                    )
                except Cadastro.DoesNotExist:
                    raise ValueError(
                        f'Cadastro com RE "{cadastro_re}" não encontrado no sistema.'
                    )
                except Cadastro.MultipleObjectsReturned:
                    raise ValueError(
                        f'Múltiplos cadastros encontrados com RE "{cadastro_re}". Corrija a base de dados.'
                    )

                # Campos obrigatórios
                data_pedido = converter_data(row.get("data_pedido"), "data_pedido")
                if not data_pedido:
                    raise ValueError('Coluna "data_pedido" inválida ou vazia.')

                status = str(row.get("status", "")).strip()
                if not status:
                    raise ValueError('Coluna "status" não pode estar vazia.')
                # Opcional: Validar contra choices do modelo
                status_choices = [
                    choice[0]
                    for choice in Cadastro_rpt._meta.get_field("status").choices
                ]
                if status not in status_choices:
                    raise ValueError(
                        f'Valor "{status}" inválido para o campo "status". Válidos: {", ".join(status_choices)}'
                    )

                sgb_destino = str(row.get("sgb_destino", "")).strip()
                if not sgb_destino:
                    raise ValueError('Coluna "sgb_destino" não pode estar vazia.')
                # Opcional: Validar SGB

                posto_secao_destino = str(row.get("posto_secao_destino", "")).strip()
                if not posto_secao_destino:
                    raise ValueError(
                        'Coluna "posto_secao_destino" não pode estar vazia.'
                    )
                # Opcional: Validar Posto/Seção

                doc_solicitacao = str(row.get("doc_solicitacao", "")).strip()
                if not doc_solicitacao:
                    raise ValueError('Coluna "doc_solicitacao" não pode estar vazia.')

                # Campos opcionais
                data_movimentacao = converter_data(
                    row.get("data_movimentacao"), "data_movimentacao"
                )  # Permite None
                data_alteracao = converter_data(
                    row.get("data_alteracao"), "data_alteracao"
                )  # Permite None
                ...
                def clean_optional_text(value):
                    ...
                    return (
                        val
                        if val
                        and val.lower() not in ["na", "n/a", "nan", "none", "#n/d"]
                        else None
                    )
        ...
        if registros_processados > 0:
            messages.success(
                request,
                f"✅ {registros_processados} registro(s) RPT importado(s) com sucesso!",
            )

        if erros_processamento:
            total_erros = len(erros_processamento)
            erros_preview = "; ".join(
                erros_processamento[:5]
