...
class Cadastro_adicional(models.Model):
    ...
    numero_adicional = models.PositiveSmallIntegerField(
        choices=N_CHOICES, verbose_name="Número do Adicional"
    )
    ...
    numero_prox_adicional = models.PositiveSmallIntegerField(
        choices=N_CHOICES, default=1, verbose_name="Próximo Número do Adicional"
    )
    proximo_adicional = models.DateField(
        null=True, blank=True, verbose_name="Próximo Adicional"
    )
    mes_proximo_adicional = models.PositiveSmallIntegerField(
        null=True, blank=True, verbose_name="Mês do Próximo Adicional"
    )
    ano_proximo_adicional = models.PositiveSmallIntegerField(
        null=True, blank=True, verbose_name="Ano do Próximo Adicional"
    )
    ...
    data_concessao_adicional = models.DateField(
        null=True, blank=True, verbose_name="Data de Concessão do Adicional"
    )
    bol_g_pm_adicional = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="BOL GPm Adicional"
    )
    data_publicacao_adicional = models.DateField(
        null=True, blank=True, verbose_name="Data Publicação Adicional"
    )
    ...
    def clean(self):
        ...
        if self.numero_prox_adicional > 8:
            raise ValidationError(
                {
                    "numero_prox_adicional": "O próximo número do adicional não pode ser maior que 8"
                }
            )
        ...
        if self.numero_prox_adicional < 1:
            raise ValidationError(
                {
                    "numero_prox_adicional": "O próximo número do adicional não pode ser menor que 1"
                }
            )
    ...
    def usuario_conclusao_display(self):
        if self.usuario_conclusao and hasattr(self.usuario_conclusao, "profile"):
            ...
            return (
                self.usuario_conclusao.get_full_name()
                or self.usuario_conclusao.username
            )
        return "-"
    ...
    def status_adicional_display(self):
        ...
        if self.situacao_adicional == "Concluído":
            if self.data_concessao_adicional:
                return mark_safe(
                    f'<span class="bg-purple-500 text-white px-2 py-1 rounded">Concluído em {self.data_concessao_adicional.strftime("%d/%m/%Y")}</span>'
                )
            return mark_safe(
                '<span class="bg-purple-500 text-white px-2 py-1 rounded">Concluído</span>'
            )
        ...
        if self.proximo_adicional:
            ...
            if diferenca > 30:
                return mark_safe(
                    '<span class="bg-green-500 text-white px-2 py-1 rounded">Aguardar</span>'
                )
            elif 0 <= diferenca <= 30:
                return mark_safe(
                    '<span class="bg-yellow-500 text-white px-2 py-1 rounded">Lançar</span>'
                )
            else:
                return mark_safe(
                    '<span class="bg-red-500 text-white px-2 py-1 rounded">Vencido</span>'
                )
    ...
    def pode_concluir(self, user):
        ...
        return (
            user.has_perm("adicional.can_concluir_adicional")
            and not self.is_concluido
            and self.status_adicional
            in ["faz_jus", "lancado_sipa", "aguardando_publicacao", "publicado"]
        )
...
class HistoricoCadastro(models.Model):
    ...
    numero_adicional = models.PositiveSmallIntegerField(
        choices=N_CHOICES, verbose_name="Número do Adicional", default=1
    )
    data_ultimo_adicional = models.DateField(
        verbose_name="Data do Último Adicional", default=timezone.now
    )
    numero_prox_adicional = models.PositiveSmallIntegerField(
        choices=N_CHOICES, verbose_name="Próximo Número do Adicional"
    )
    proximo_adicional = models.DateField(
        null=True, blank=True, verbose_name="Próximo Adicional"
    )
