...
class CadastroAdicionalAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            "Identificação",
            {"fields": ("link_cadastro", "numero_adicional", "numero_prox_adicional")},
        ),
        (
            "Datas Chave",
            {
    ...
    def acoes(self, obj):
        return format_html(
            ...
            reverse("admin:adicional_historicocadastro_changelist")
            + f"?cadastro_adicional__id__exact={obj.id}",
        )
...
class HistoricoCadastroAdmin(admin.ModelAdmin):
    ...
    def cadastro_adicional_link(self, obj):
        url = reverse(
            "admin:adicional_cadastro_adicional_change",
            args=[obj.cadastro_adicional.id],
        )
