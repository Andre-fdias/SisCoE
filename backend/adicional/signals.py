...
def check_adicional_status(sender, instance, created, **kwargs):
    ...
    try:
        ...
        if (instance.status_adicional == Cadastro_adicional.StatusAdicional.AGUARDANDO_REQUISITOS and
            instance.proximo_adicional and
            hoje >= instance.proximo_adicional):

            # Somente atualiza se o status for realmente diferente do que será salvo.
            # Isso evita saves desnecessários e potencial loop.
            if instance.status_adicional != Cadastro_adicional.StatusAdicional.FAZ_JUS: # Assumindo que VENCIDO_PENDENTE significa FAZ_JUS
                ...
                with disable_signals(): # Use o gerenciador de contexto para evitar recursão
                    instance.save(update_fields=['status_adicional', 'updated_at', 'user_updated'])
                logger.info(f"Status do Adicional ID {instance.id} atualizado para 'Faz Jus' devido à data.")

    except Exception as e:
        logger.error(f"Erro ao verificar status para Adicional ID {instance.id}: {e}", exc_info=True)
    finally:
        if hasattr(instance, '_checking_status_signal_active'):
