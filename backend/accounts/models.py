...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    login_history = models.JSONField(_('Login History'), default=list, blank=True, null=True)
    ...
    def get_user_sgb(self):
        ...
        if not hasattr(self, '_user_sgb_cache'):
            ...
            if self.cadastro:
                try:
                    latest_situacao = self.cadastro.detalhes_situacao.order_by('-data_alteracao').first()
                    if latest_situacao:
    ...
    def update_login_history(self, ip, computer_name, login_time=None, logout_time=None):
        ...
        if login_time:
            history.append({
                'login_time': login_time.isoformat() if not isinstance(login_time, str) else login_time,
                'ip': ip,
            ...
            for entry in reversed(history):
                if entry.get('logout_time') is None:
                    entry['logout_time'] = logout_time.isoformat() if not isinstance(logout_time, str) else logout_time
                    break
