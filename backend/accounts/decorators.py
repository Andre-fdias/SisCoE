        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if request.user.permissoes == 'visitantes':
                ...
                if view_name not in allowed_views_for_visitantes:
                    messages.error(request, "Visitantes não têm permissão para acessar esta página.")
                    return redirect(reverse(redirect_url))
            
            # Verificação normal de permissão hierárquica
            if not request.user.has_permission_level(level):
                messages.error(request, f"Você precisa ter permissão de {level} para acessar esta página.")
                return redirect(reverse(redirect_url))
...
def apply_model_permissions_filter(model_class, redirect_url='acesso_negado'):
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if queryset.count() == 0 and not (request.user.is_superuser or request.user.has_permission_level('gestor')):
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not (request.user.is_superuser or request.user.has_perm(perm)):
                messages.error(request, f"Você não tem a permissão '{perm}' necessária.")
                return redirect(reverse(redirect_url))
...
def group_required(*group_names, redirect_url='acesso_negado'):
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not (request.user.is_superuser or request.user.groups.filter(name__in=group_names).exists()):
                groups = ", ".join(group_names)
                messages.error(request, f"Você precisa pertencer ao grupo {groups} para acessar.")
                return redirect(reverse(redirect_url))
