...
from backend.efetivo.models import (
    Cadastro,
    DetalhesSituacao,
)  # Certifique-se de que Imagem está importada
...
)  # CustomUserCreationForm não será usado diretamente para criar o User aqui
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not cadastro:
                messages.error(
                    request,
                    "CPF não encontrado na base de dados. Por favor, entre em contato com o administrador para ser cadastrado.",
                )
            ...
            if User.objects.filter(cadastro=cadastro).exists():
                messages.warning(
                    request,
                    "Este CPF já possui uma conta de usuário. Por favor, faça login ou redefina sua senha.",
                )
            ...
            if (
                ...
                messages.error(
                    request,
                    "Seu CPF não atende aos requisitos de situação para cadastro. Por favor, entre em contato com o administrador.",
                )
                logger.debug(
            ...
            if hasattr(cadastro, "image") and cadastro.image:
                try:
                    ...
                    logger.warning(
                        f"Não foi possível processar a imagem do Cadastro {cadastro.id} para pré-visualização: {e}"
                    )
...
def signup(request):
    ...
    if not cadastro_data:
        messages.error(
            request,
            "Sessão de cadastro expirada ou inválida. Por favor, verifique seu CPF novamente.",
        )
    ...
    if request.method == "POST":
        ...
        if not terms_accepted or not signature_data:
            messages.error(
                request,
                "Você deve aceitar os Termos e Condições e fornecer sua assinatura para criar sua conta.",
            )
            return render(
                request,
        ...
        try:
            ...
            if User.objects.filter(cadastro=cadastro_obj).exists():
                messages.error(
                    request,
                    "Já existe uma conta associada a este cadastro militar. Por favor, faça login ou recupere sua senha.",
                )
            ...
            if (
                ...
                messages.error(
                    request,
                    "Seu status militar não permite o cadastro no momento. Por favor, entre em contato com o administrador.",
                )
...
def change_password_view(request, pk):
    ...
    if not request.user.pk == user_to_change.pk:
        messages.error(
            request, "Você não tem permissão para alterar a senha de outro usuário."
        )
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if user.must_change_password:
                    login(
                        request, user
                    )  # Adicione esta linha para autenticar o usuário antes do redirecionamento
                    messages.info(
                ...
                if hasattr(user, "cadastro") and user.cadastro:
                    ...
                    if (
                        ...
                        messages.error(
                            request,
                            "Sua conta está inativa ou não atende aos requisitos de acesso. Por favor, entre em contato com o administrador.",
                        )
                        # Redireciona para landing com parâmetro para abrir modal de login
                        return redirect(reverse("core:capa") + "?open_login_modal=true")
                else:
                    ...
                    messages.error(
                        request,
                        "Sua conta não está associada a um cadastro válido. Por favor, entre em contato com o administrador.",
                    )
                    # Redireciona para landing com parâmetro para abrir modal de login
                    return redirect(reverse("core:capa") + "?open_login_modal=true")
            else:
...
def my_logout(request):
    ...
    return redirect("core:capa")  # Redireciona para a página de login
...
def user_list(request):
    ...
    if not request.user.has_permission_level(
        ...
        messages.error(
            request, "Você não tem permissão para visualizar a lista de usuários."
        )
...
def generate_termo_pdf(request, pk):
    ...
    p = Paragraph(
        "O acesso ao sistema é exclusivo para fins institucionais, em conformidade com as atribuições funcionais do usuário e com as disposições da Instrução Policial-Militar I-31-PM (3ª edição, 2022) e da Lei Geral de Proteção de Dados (LGPD - Lei nº 13.709/2018).",
        styles["Justify"],
    )
    ...
    p = Paragraph(
        "Os dados pessoais tratados no sistema estão sujeitos às normas da LGPD, incluindo os princípios de finalidade, adequação, necessidade e transparência. O usuário consente com o tratamento de seus dados pessoais para fins de controle de acesso, auditoria e cumprimento de obrigações legais.",
        styles["Justify"],
    )
    p_width, p_height = p.wrap(width - 2 * margin, height)
    p.drawOn(pdf, margin, y_position - p_height)
    ...
    p = Paragraph(
        "A PMESP reserva-se o direito de monitorar e auditar o uso do sistema, conforme previsto no Artigo 7º da I-31-PM. Em caso de irregularidades, o usuário estará sujeito às sanções administrativas, civis e penais cabíveis.",
        styles["Justify"],
    )
    p_width, p_height = p.wrap(width - 2 * margin, height)
    p.drawOn(pdf, margin, y_position - p_height)
    ...
    p = Paragraph(
        "Descumprimento deste termo acarretará medidas disciplinares, conforme o Regulamento Geral da PMESP e a legislação vigente, podendo resultar em suspensão ou cancelamento do acesso, ações disciplinares e medidas legais cabíveis.",
        styles["Justify"],
    )
    p_width, p_height = p.wrap(width - 2 * margin, height)
    p.drawOn(pdf, margin, y_position - p_height)
    ...
    for item in disposicoes:
        pdf.setFont("Helvetica", 9)
        pdf.drawString(
            margin if not item.startswith("•") else margin + 10, y_position, item
        )
        y_position -= line_height
...
def access_history(request, pk):
    ...
    if not (request.user == user_obj or request.user.is_superuser):
        messages.error(
            request, "Não tem permissão para visualizar este histórico de acessos."
        )
    ...
    log_user_action(
        request.user, f"Visualizou o histórico de acessos de {user_obj.email}", request
    )
...
def user_action_history(request, pk):
    ...
    if not (request.user == user_obj or request.user.is_superuser):
        messages.error(
            request, "Não tem permissão para visualizar este histórico de ações."
        )
        return redirect("accounts:user_detail", pk=user_obj.pk)
    ...
    log_user_action(
        request.user, f"Visualizou o histórico de ações de {user_obj.email}", request
    )
...
def user_permission_update(request, pk):
    ...
    if request.user == user_to_edit:
        ...
        messages.error(
            request, "Você não pode alterar suas próprias permissões por esta tela."
        )
    ...
    if not request.user.is_superuser:
        if user_to_edit.is_superuser:
            ...
            messages.error(
                request,
                "Você não tem permissão para alterar as permissões de um superusuário.",
            )
        ...
        if (
            ...
            messages.error(
                request,
                "Você não tem permissão para alterar as permissões de um administrador.",
            )
            return redirect("accounts:user_list")
        ...
        if not request.user.has_permission_level("gestor"):
            ...
            messages.error(
                request,
                "Você não tem permissão para acessar a gestão de permissões de usuários.",
            )
            return redirect("accounts:user_list")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Permissões de {user.email} atualizadas com sucesso!"
            )
            return redirect("accounts:user_list")
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
                {"error": "Erro de validação", "errors": form.errors}, status=400
            )
...
def global_access_history(request):
    if not request.user.has_permission_level("gestor"):
        messages.error(
            request,
            "Você não tem permissão para visualizar o histórico de acessos de todos os usuários.",
        )
        return redirect("core:index")
...
def global_user_action_history(request):
    if not request.user.has_permission_level("gestor"):
        messages.error(
            request,
            "Você não tem permissão para visualizar o histórico de ações de todos os usuários.",
        )
        return redirect("core:index")
