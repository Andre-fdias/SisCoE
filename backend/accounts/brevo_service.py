    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'IP' in str(error_data):
                logger.critical("🚨 IP não autorizado! Adicione o IP em: https://app.brevo.com/security/authorised_ips")
            return False
        else:
...
def send_brevo_email(subject, html_content, to_email, from_email=None, from_name=None, max_retries=2):
    """
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if response.status_code == 401:
                error_data = response.json()
                if 'unauthorized' in error_data.get('message', '').lower() and 'IP' in error_data.get('message', ''):
                    logger.error("❌ ERRO CRÍTICO: IP não autorizado no Brevo")
                    logger.error(f"🔧 Solução: Adicione o IP 177.84.247.135 em: https://app.brevo.com/security/authorised_ips")
                    return False
            ...
            error_msg = f"❌ Tentativa {attempt + 1} - Falha ao enviar e-mail para {to_email}"
            ...
            if hasattr(e, 'response') and e.response is not None:
                ...
                try:
                    ...
                    if e.response.status_code == 401 and 'IP' in str(error_details):
                        ...
                        logger.critical("🔧 Acesse: https://app.brevo.com/security/authorised_ips")
