...
from django.contrib.auth.forms import UserCreationForm, UserChangeForm, PasswordChangeForm as AuthPasswordChangeForm, AuthenticationForm
...
class CustomAuthenticationForm(AuthenticationForm):
    ...
    username = forms.EmailField(
        label="Email",
        widget=forms.EmailInput(attrs={'autofocus': True, 'class': 'form-input', 'placeholder': 'Seu Email'})
    )
...
class CustomUserCreationForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ('email', 'first_name', 'last_name', 'permissoes') # Adicione 'permissoes' aqui
...
class CustomUserChangeForm(UserChangeForm):
    ...
    class Meta:
        model = User
        fields = ('email', 'first_name', 'last_name', 'is_active', 'is_admin', 'permissoes', 'must_change_password')
        field_classes = {'email': forms.EmailField}
...
class PasswordChangeForm(AuthPasswordChangeForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['old_password'].widget = PasswordInput(attrs={'class': 'form-input', 'placeholder': 'Senha Antiga'})
        self.fields['new_password1'].widget = PasswordInput(attrs={'class': 'form-input', 'placeholder': 'Nova Senha'})
        self.fields['new_password2'].widget = PasswordInput(attrs={'class': 'form-input', 'placeholder': 'Confirme a Nova Senha'})
...
class ForcePasswordChangeForm(forms.Form):
    ...
    new_password1 = forms.CharField(
        label="Nova Senha",
        widget=forms.PasswordInput(attrs={'class': 'form-input', 'placeholder': 'Nova Senha'}),
        strip=False,
...
class UserPermissionChangeForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ('permissoes', 'is_active', 'is_admin', 'is_superuser', 'must_change_password')

    def __init__(self, *args, **kwargs):
        current_user = kwargs.pop('current_user', None) # O usuário que está fazendo a edição
        super().__init__(*args, **kwargs)
        ...
        if self.instance and self.instance.is_superuser:
            ...
            if current_user and not current_user.is_superuser:
                for field_name in self.fields:
                    ...
                    self.fields[field_name].help_text = "Permissões de superusuários só podem ser alteradas por outro superusuário."
            ...
            if self.instance == current_user or (self.instance.is_superuser and current_user and current_user.is_superuser):
                ...
                self.fields['is_superuser'].help_text = "Este campo não pode ser alterado diretamente por este formulário."
                self.fields['is_admin'].help_text = "Este campo não pode ser alterado diretamente por este formulário."
                self.fields['permissoes'].help_text = "Este campo não pode ser alterado diretamente para superusuários."

        # Se o usuário a ser editado NÃO é um superusuário
        else:
            # Um gestor não pode alterar o status de admin ou superusuário
            if current_user and current_user.has_permission_level('gestor') and not current_user.is_superuser:
                if 'is_admin' in self.fields:
                    self.fields['is_admin'].widget.attrs['disabled'] = 'disabled'
                    self.fields['is_admin'].help_text = "Você não tem permissão para alterar o status de administrador."
                if 'is_superuser' in self.fields:
                    self.fields['is_superuser'].widget.attrs['disabled'] = 'disabled'
                    self.fields['is_superuser'].help_text = "Você não tem permissão para alterar o status de superusuário."
            
            # Um usuário não pode alterar suas próprias permissões ou status
            if self.instance == current_user:
                for field_name in self.fields:
                    self.fields[field_name].widget.attrs['disabled'] = 'disabled'
                    self.fields[field_name].help_text = "Você não pode alterar suas próprias permissões ou status por esta tela."
