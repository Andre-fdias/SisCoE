...
user_management_patterns = [
    ...
    path(
        "<int:pk>/access_history/", v.access_history, name="access_history"
    ),  # URL para histórico de acessos
    path(
        "<int:pk>/action_history/", v.user_action_history, name="user_action_history"
    ),  # URL para histórico de ações
    path(
        "<int:pk>/change_password/", v.change_password_view, name="change_password_view"
    ),
    path(
        "<int:pk>/permission_update/",
        v.user_permission_update,
...
urlpatterns = [
    ...
    path(
        "register/", v.signup, name="signup"
    ),  # Agora esta é a view de confirmação/criação
    path(
        "reset/<uidb64>/<token>/",
    ...
    path(
        "global_access_history/", v.global_access_history, name="global_access_history"
    ),
    path(
        "global_action_history/",
        v.global_user_action_history,
        name="global_user_action_history",
    ),
    path("verificar-cpf/", v.verificar_cpf, name="verificar_cpf"),
    # NOVA ROTA: Troca de senha forçada no primeiro login
    path(
        "force-password-change/",
        v.force_password_change_view,
        name="force_password_change",
    ),
    path(
        "admin-login/", v.admin_login, name="admin_login"
    ),  # Adicionada para consistência
]
