...
from .models import LP, HistoricoLP, N_CHOICES
...
import logging
import json
from django.db.models import Subquery, OuterRef  # Importar Max, Subquery, OuterRef

logger = logging.getLogger(__name__)


def alert_response(
    type, title, message, status=200, reload_page=False, redirect_url=None, errors=None
):
    """
    ...
    """
    response_data = {"alert": {"type": type, "title": title, "message": message}}
    if reload_page:
    ...
    if ultima_lp:
        ...
        if not cadastro.data_ingresso:
            raise ValidationError(
                "Data de ingresso do militar não definida no cadastro."
            )
        data_inicio_aquisitivo_atual = cadastro.data_ingresso
        proximo_numero_lp = 1

    data_fechamento_bloco_base = (
        data_inicio_aquisitivo_atual + relativedelta(years=5) - timedelta(days=1)
    )
        ...
        return JsonResponse(
            {
                ...
                "alert": {
                    "type": "error",
                    "title": "Erro!",
                    "message": "O RE do militar é obrigatório.",
                },
            },
            status=400,
        )
            ...
            return JsonResponse(
                {
                    ...
                    "alert": {
                        "type": "error",
                        "title": "Erro!",
                        "message": "Detalhamento da situação não encontrado para este militar.",
                    },
                },
                status=400,
            )
            ...
            return JsonResponse(
                {
                    ...
                    "alert": {
                        "type": "error",
                        "title": "Erro!",
                        "message": "Dados de Posto/Graduação não encontrados para este militar.",
                    },
                },
                status=400,
            )
        ...
        return JsonResponse(
            {
                ...
                "alert": {
                    "type": "error",
                    "title": "Erro!",
                    "message": f'Militar com RE "{re_militar}" não cadastrado no sistema.',
                },
            },
            status=404,
        )
        ...
        return JsonResponse(
            {
                ...
                "alert": {
                    "type": "error",
                    "title": "Erro Interno!",
                    "message": f"Ocorreu um erro inesperado ao buscar o militar: {str(e)}",
                },
            },
            status=500,
        )
...
def ver_lp(request, pk):
    ...
    latest_concluded_history_dates = (
        HistoricoLP.objects.filter(
    ...
    if lp.status_lp == LP.StatusLP.AGUARDANDO_REQUISITOS:
        ...
        if lp.data_ultimo_lp:
            ...
            data_fim_periodo_lp_calculated = (
                data_inicio_periodo_lp_calculated
    ...
    if lp.data_ultimo_lp:
        ...
        data_fim_periodo_lp = (
            data_inicio_periodo_lp + relativedelta(years=5) - timedelta(days=1)
        )

        if hoje > data_fim_periodo_lp:
            dias_decorridos_periodo_lp = (
                data_fim_periodo_lp - data_inicio_periodo_lp
            ).days
            dias_restantes_periodo_lp = 0
            ...
            if total_dias_periodo_lp > 0:
                progresso_periodo_aquisitivo_percentual = (
    ...
    if hasattr(lp, "fruicao"):
        ...
        fruicao_data_for_js = {
            ...
            "data_concessao_lp": (
                fruicao_obj.data_concessao_lp.strftime("%Y-%m-%d")
                if fruicao_obj.data_concessao_lp
                else None
            ),
            "bol_g_pm_lp": fruicao_obj.bol_g_pm_lp,
            "data_publicacao_lp": (
                fruicao_obj.data_publicacao_lp.strftime("%Y-%m-%d")
                if fruicao_obj.data_publicacao_lp
                else None
            ),
    ...
    context = {
        ...
        "progresso_periodo_aquisitivo_percentual": round(
            progresso_periodo_aquisitivo_percentual, 2
        ),
...
def editar_concessao_lp(request, pk):
    ...
    try:
        ...
        # Conversão de datas
        data_concessao_lp = (
            date.fromisoformat(data_concessao_lp_str) if data_concessao_lp_str else None
        )
        data_publicacao_lp = (
            date.fromisoformat(data_publicacao_lp_str)
        ...
        if is_ajax:
            return alert_response(
                "success",
                "Sucesso!",
                "Concessão da LP atualizada com sucesso!",
                reload_page=True,
            )
        ...
        if is_ajax:
            return alert_response(
                "error",
                "Erro Interno",
                "Ocorreu um erro inesperado ao editar a concessão da LP.",
                500,
            )
        messages.error(
            request,
            "Ocorreu um erro inesperado ao editar a concessão da LP. O administrador foi notificado.",
        )
    ...
    try:
        ...
        if dias_desconto_lp < 0:
            if is_ajax:
                return alert_response(
                    "error",
                    "Erro de Validação!",
                    "Os dias de desconto não podem ser negativos.",
                    400,
                )
        ...
        with transaction.atomic():
            ...
            if lp.data_ultimo_lp:
                ...
                proximo_lp_calculado = (
                    lp.data_ultimo_lp
                    + timedelta(days=1)
                    + timedelta(days=lp.dias_desconto_lp)
                )
        ...
        if is_ajax:
            return alert_response(
                "success",
                "Sucesso!",
                "Dias de desconto da LP atualizados com sucesso!",
                reload_page=True,
            )
    ...
    try:
        ...
        if not password:
            if is_ajax:
                return alert_response(
                    "error",
                    "Erro de Validação!",
                    "A senha é obrigatória para excluir.",
                    400,
        ...
        if is_ajax:
            return alert_response(
                "error",
                "Erro Interno",
                "Ocorreu um erro inesperado ao excluir a LP.",
                500,
            )
        messages.error(
            request,
            "Ocorreu um erro inesperado ao excluir a LP. O administrador foi notificado.",
        )
...
def listar_lp(request):
    ...
    context = {"lps": lps, "N_CHOICES": N_CHOICES}
...
def concluir_lp(request, pk):
    ...
    try:
        ...
        logger.error(
            f"ValidationError em concluir_lp para LP ID {pk}: {e.message_dict}",
            exc_info=True,
        )
        return alert_response(
            "error", "Erro de Validação!", error_msg, 400, errors=e.message_dict
        )
    except Exception as e:
        error_msg = (
            f"Ocorreu um erro inesperado: {str(e)}. O administrador foi notificado."
        )
    ...
    try:
        ...
        return alert_response(
            ...
            errors=(
                ...
                else {"__all__": e.messages}
            ),
        )
    except Exception as e:
        logger.error(f"Erro ao criar LP: {str(e)}", exc_info=True)
        return alert_response(
            "error", "Erro Interno", f"Erro ao criar LP: {str(e)}", 500
        )
...
def editar_lp(request, pk):
    ...
    if not is_ajax:
        messages.error(
            request, "Requisição inválida. Por favor, use o formulário de edição."
        )
        return redirect("lp:ver_lp", pk=pk)
    ...
    try:
        ...
        return alert_response(
            "success", "Sucesso!", "LP atualizada com sucesso!", reload_page=True
        )
        ...
        return alert_response(
            "error", "Erro Interno", "Ocorreu um erro inesperado ao editar a LP.", 500
        )
    ...
    try:
        ...
        if is_ajax:
            return alert_response(
                "success",
                "Sucesso!",
                "Lançamento no SIPA confirmado com sucesso!",
                reload_page=True,
            )
        ...
        if is_ajax:
            return alert_response(
                "error",
                "Erro Interno",
                "Ocorreu um erro inesperado ao confirmar o lançamento no SIPA.",
                500,
            )
        messages.error(
            request,
            "Ocorreu um erro inesperado ao confirmar o lançamento no SIPA. O administrador foi notificado.",
        )
...
def carregar_dados_sipa_lp(request, pk):
    ...
    data = {
        "dias_desconto_lp": lp.dias_desconto_lp,
        "data_concessao_lp": (
            lp.data_concessao_lp.strftime("%Y-%m-%d") if lp.data_concessao_lp else ""
        ),
        "numero_lp": lp.numero_lp,
...
def detalhar_fruicao(request, pk):
    ...
    historico = fruicao.historico.filter(
        data_inicio_afastamento__isnull=False, data_termino_afastamento__isnull=False
    ).order_by("-data_alteracao")
    ...
    if request.method == "POST":
        ...
        if dias_afastamento:
            if fruicao_instance.dias_disponiveis < dias_afastamento:
                messages.error(
                    request, "Dias disponíveis insuficientes para este afastamento."
                )
                return render(
                    request,
                    "fruicao/_adicionar_afastamento_modal.html",
                    {
            ...
            if data_inicio:
                fruicao_instance.data_inicio_afastamento = datetime.strptime(
                    data_inicio, "%Y-%m-%d"
                ).date()
            if data_termino:
                fruicao_instance.data_termino_afastamento = datetime.strptime(
                    data_termino, "%Y-%m-%d"
                ).date()
    ...
    data = {
        ...
        "registro": {
            ...
            "data_inicio_afastamento": (
                registro.data_inicio_afastamento.strftime("%d/%m/%Y")
                if registro.data_inicio_afastamento
                else None
            ),
            "data_termino_afastamento": (
                registro.data_termino_afastamento.strftime("%d/%m/%Y")
                if registro.data_termino_afastamento
                else None
            ),
        },
    }
...
def remover_afastamento(request, pk, afastamento_id):
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"Erro ao remover afastamento: {str(e)}"},
                status=500,
            )
