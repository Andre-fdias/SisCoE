...
class LP(models.Model):
    ...
    cadastro = models.ForeignKey(
        "efetivo.Cadastro", on_delete=models.CASCADE, verbose_name="Cadastro"
    )
    ...
    numero_lp = models.PositiveSmallIntegerField(
        choices=N_CHOICES, verbose_name="Número da LP"
    )
    data_ultimo_lp = models.DateField(
        null=True, blank=True, verbose_name="Data do Último LP"
    )
    numero_prox_lp = models.PositiveSmallIntegerField(
        choices=N_CHOICES, verbose_name="Próximo Número da LP", null=True, blank=True
    )
    proximo_lp = models.DateField(null=True, blank=True, verbose_name="Próximo LP")
    mes_proximo_lp = models.PositiveSmallIntegerField(
        null=True, blank=True, verbose_name="Mês do Próximo LP"
    )
    ano_proximo_lp = models.PositiveSmallIntegerField(
        null=True, blank=True, verbose_name="Ano do Próximo LP"
    )
    ...
    data_concessao_lp = models.DateField(
        null=True, blank=True, verbose_name="Data de Concessão da LP"
    )
    ...
    def clean(self):
        ...
        if self.data_ultimo_lp and self.data_ultimo_lp > timezone.localdate():
            raise ValidationError(
                {"data_ultimo_lp": "A data do último LP não pode ser futura."}
            )
        ...
        if self.data_concessao_lp and self.data_concessao_lp > timezone.localdate():
            raise ValidationError(
                {"data_concessao_lp": "A data de concessão não pode ser futura."}
            )
        ...
        if self.data_publicacao_lp and self.data_publicacao_lp > timezone.localdate():
            raise ValidationError(
                {"data_publicacao_lp": "A data de publicação não pode ser futura."}
            )

        # Validação para garantir que data_publicacao_lp não é anterior a data_concessao_lp
        if (
            self.data_concessao_lp
            and self.data_publicacao_lp
            and self.data_publicacao_lp < self.data_concessao_lp
        ):
            raise ValidationError(
                {
                    "data_publicacao_lp": "A data de publicação não pode ser anterior à data de concessão."
                }
            )
...
class HistoricoLP(models.Model):
    ...
    situacao_lp = models.CharField(
        max_length=30, choices=situacao_choices, verbose_name="Situação da LP"
    )
    status_lp = models.CharField(
        max_length=30, choices=LP.StatusLP.choices, verbose_name="Status da LP"
    )
    ...
    data_conclusao = models.DateTimeField(
        null=True, blank=True, verbose_name="Data de Conclusão (Histórico)"
    )
    observacoes_historico = models.TextField(
...
class LP_fruicao(models.Model):
    ...
    lp_concluida = models.OneToOneField(
        LP,
    ...
    bol_g_pm_lp = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="BOL G PM"
    )
    data_publicacao_lp = models.DateField(
        null=True, blank=True, verbose_name="Data Publicação LP"
    )
    ...
    dias_disponiveis = models.PositiveSmallIntegerField(
        default=90, verbose_name="Dias Disponíveis"
    )
    dias_utilizados = models.PositiveSmallIntegerField(
        default=0, verbose_name="Dias Utilizados"
    )
    data_inicio_afastamento = models.DateField(
        null=True, blank=True, verbose_name="Data de Início do Afastamento"
    )
    data_termino_afastamento = models.DateField(
        null=True, blank=True, verbose_name="Data de Término do Afastamento"
    )
    bol_int = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="BOL Int"
    )
    data_bol_int = models.DateField(
        null=True, blank=True, verbose_name="Data do BOL Int"
    )
    ...
    def clean(self):
        if self.data_inicio_afastamento and self.data_termino_afastamento:
            ...
            if (
                ...
                and self.tipo_periodo_afastamento != dias_calculados
            ):
                raise ValidationError(
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            ...
            if any(
                getattr(old_instance, campo) != getattr(self, campo)
    ...
    def get_search_result(self):
        return {
            ...
            "fields": {
                ...
                "Início Afastamento": (
                    self.data_inicio_afastamento.strftime("%d/%m/%Y")
                    if self.data_inicio_afastamento
                    else "-"
                ),
            },
        }
...
class HistoricoFruicaoLP(models.Model):
    ...
    fruicao = models.ForeignKey(
        LP_fruicao, on_delete=models.CASCADE, related_name="historico"
    )
    ...
    class Meta:
        ordering = [
            "-data_alteracao"
        ]  # Garante que os registros mais recentes apareçam primeiro
    ...
    def criar_registro(cls, fruicao):
        return cls.objects.create(
            fruicao=fruicao,
            usuario=getattr(fruicao, "user_updated", None)
            or getattr(fruicao, "user_created", None),
