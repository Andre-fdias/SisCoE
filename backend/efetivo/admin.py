...
class StatusInatividadeFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "inativo":
            ...
            return queryset.filter(
                previsao_de_inatividade__gt=hoje, previsao_de_inatividade__lte=um_ano
            )
        elif self.value() == "mais_1_ano":
            return queryset.filter(
                previsao_de_inatividade__gt=hoje + timezone.timedelta(days=365)
            )
...
class ImagemInline(admin.TabularInline):
    ...
    def image_tag(self, obj):
        if obj.image:
            return mark_safe(
                f'<img src="{obj.image.url}" width="50" height="50" style="border-radius: 50%;" />'
            )
        return "Sem imagem"
...
class CatEfetivoInline(admin.StackedInline):
    ...
    fields = (
        ...
        "data_termino",
        "status_badge",
        "restricoes_selecionadas_badges",
    )
...
class CadastroAdmin(admin.ModelAdmin):
    ...
    def posto_grad(self, obj):
        return (
            obj.ultima_promocao.posto_grad
            if hasattr(obj, "ultima_promocao") and obj.ultima_promocao
            else "-"
        )
...
class HistoricoCatEfetivoAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        ...
        "cat_efetivo__cadastro__nome_de_guerra",
    )
    list_per_page = 25
    readonly_fields = (
        "tipo_badge",
        "status_info_badge",
        "restricoes_selecionadas_siglas",
    )
