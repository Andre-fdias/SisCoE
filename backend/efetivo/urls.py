...
urlpatterns = [
    ...
    path(
        "lista_militares/", views.ListaMilitaresView.as_view(), name="lista_militares"
    ),
    ...
    path(
        "publico/<int:id>/",
        views.visualizar_militar_publico,
        name="visualizar_militar_publico",
    ),
    path(
        "editar_imagem_user/<int:id>/",
        views.editar_imagem_user,
        name="editar_imagem_user",
    ),
    # --- Edição de Dados Específicos de Militares ---
    path(
        "editar_posto_graduacao/<int:id>/",
        views.editar_posto_graduacao,
        name="editar_posto_graduacao",
    ),
    path(
        "editar_dados_pessoais_contatos/<int:id>/",
        views.editar_dados_pessoais_contatos,
        name="editar_dados_pessoais_contatos",
    ),
    path("editar_imagem/<int:id>/", views.editar_imagem, name="editar_imagem"),
    # urls.py
    path(
        "editar_averbacao_militar/<int:id>/",
        views.editar_averbacao_militar,
        name="editar_averbacao_militar",
    ),
    path(
        "editar_averbacao_inss/<int:id>/",
        views.editar_averbacao_inss,
        name="editar_averbacao_inss",
    ),
    path("editar_telefone/<int:id>/", views.editar_telefone, name="editar_telefone"),
    path(
        "editar_email/<int:id>/", views.editar_email, name="editar_email"
    ),  # <--- ADICIONE ESTA LINHA
    # --- Históricos e Outros Status ---
    path(
        "historico_movimentacoes/<int:id>/",
        views.historico_movimentacoes,
        name="historico_movimentacoes",
    ),
    path(
        "check_rpt/<int:id>/", views.check_rpt, name="check_rpt"
    ),  # Mantido conforme o original
    path(
        "listar_outros_status/",
        views.listar_outros_status_militar,
        name="listar_outros_status",
    ),
    # --- Gerenciamento de Categorias de Efetivo (Férias, Restrição, DS, DR, etc.) ---
    path(
        "militar/<int:militar_id>/adicionar-categoria/",
        views.adicionar_categoria_efetivo,
        name="adicionar_categoria_efetivo",
    ),
    # URL para visualizar o histórico de categorias de efetivo de um militar.
    path(
        "militar/<int:militar_id>/historico-categorias/",
        views.historico_categorias,
        name="historico_categorias",
    ),
    ...
    path(
        "excluir-categoria-efetivo/<int:categoria_id>/",
        views.excluir_categoria_efetivo,
        name="excluir_categoria_efetivo",
    ),
    # --- Novas URLs para Exclusão de Histórico de Movimentações ---
    path(
        "excluir_historico_promocao/<int:promocao_id>/",
        views.excluir_historico_promocao,
        name="excluir_historico_promocao",
    ),
    path(
        "excluir_historico_detalhe_situacao/<int:detalhe_id>/",
        views.excluir_historico_detalhe_situacao,
        name="excluir_historico_detalhe_situacao",
    ),
    ...
    path(
        ...
        views.editar_situacao_funcional,
        name="editar_situacao_funcional",
    ),
    path(
        "nova_situacao_funcional/<int:id>/",
        views.nova_situacao_funcional,
        name="nova_situacao_funcional",
    ),
]
