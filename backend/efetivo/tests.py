...
class SimpleModelTests(TestCase):
    ...
    def test_user_creation(self):
        ...
        user = User.objects.create_user(email="simple@test.com", password="testpass123")
        self.assertEqual(user.email, "simple@test.com")

    def test_cadastro_creation(self):
        """Testa criação básica de cadastro"""
        user = User.objects.create_user(
            email="cadastro@test.com", password="testpass123"
        )
    ...
    def test_detalhes_situacao_str_representation(self):
        ...
        user = User.objects.create_user(
            email="situacao@example.com", password="testpass123"
        )
        ...
        detalhes = DetalhesSituacao.objects.create(
            cadastro=cadastro, situacao="Efetivo", sgb="EM", usuario_alteracao=user
        )
...
class PropertyTests(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="props@example.com", password="testpass123"
        )
...
class PublicViewTests(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="public@example.com", password="testpass123"
        )
        self.client = Client()
...
class MetricTests(TestCase):
    ...
    def test_signal_handlers(self):
        ...
        user = User.objects.create_user(
            email="metrics@example.com", password="testpass123"
        )
...
class APITests(BaseTestCase):
    ...
    def test_check_rpt(self):
        ...
        response = self.client.get(
            reverse("efetivo:check_rpt", args=[self.cadastro.id])
        )
        self.assertEqual(response.status_code, 200)
...
class FunctionalTests(TestCase):
    ...
    def test_cadastro_search_result(self):
        ...
        user = User.objects.create_user(
            email="search@example.com", password="testpass123"
        )
