...
def editar_bm(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            cadastro.admissao = datetime.strptime(request.POST.get('admissao'), '%Y-%m-%d').date()
            cadastro.nasc = datetime.strptime(request.POST.get('nasc'), '%Y-%m-%d').date()
            cadastro.email = request.POST.get('email')
            cadastro.telefone = request.POST.get('telefone')
            cadastro.apresentacao_na_unidade = datetime.strptime(request.POST.get('apresentacao_na_unidade'), '%Y-%m-%d').date()
            saida_da_unidade_str = request.POST.get('saida_da_unidade')
            if saida_da_unidade_str:
                cadastro.saida_da_unidade = datetime.strptime(saida_da_unidade_str, '%Y-%m-%d').date()
            else:
...
def excluir_bm(request, pk):
    ...
    if request.method == "POST":
        ...
        username_field = User.USERNAME_FIELD #Get the username field from the user model.
    ...
    if request.method == 'POST' and request.FILES.get('arquivo'):
        ...
        try:
            ...
            colunas_obrigatorias = {
                'nome', 'nome_de_guerra', 'cpf', 'rg', 
                'admissao', 'nasc', 'apresentacao_na_unidade', 
                'posto_secao', 'sgb'
            }
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    apresentacao = converter_data(row['apresentacao_na_unidade'], 'apresentacao_na_unidade')
                    
                    # Tratar data de saída (opcional)
                    saida = None
                    if str(row.get('saida_da_unidade', 'N/A')) != 'N/A':
                        saida = converter_data(row['saida_da_unidade'], 'saida_da_unidade')
