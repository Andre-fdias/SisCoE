...
class ParticipantSerializer(serializers.ModelSerializer):
    ...
    def get_unread_count(self, obj):
        if obj.last_read_message:
            return (
                obj.conversation.messages.filter(
                    created_at__gt=obj.last_read_message.created_at
                )
                .exclude(sender=obj.user)
                .count()
            )
    ...
    def create(self, validated_data):
        ...
        if not is_group and len(participants_ids) == 2:
            conversations = (
                ...
                .filter(participants__user_id=participants_ids[0])
                .filter(participants__user_id=participants_ids[1])
            )
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def get_situacao(self, obj):
        ...
        if hasattr(obj, "cadastro") and obj.cadastro:
            situacao = obj.cadastro.detalhes_situacao.order_by(
                "-data_alteracao"
            ).first()
            if situacao:
