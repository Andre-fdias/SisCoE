...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_message_send(self, data):
        ...
        await self.channel_layer.group_send(
            self.conversation_group_name,
            {"type": "message.receive", "message": serialized_message},
        )
    ...
    async def handle_message_edit(self, data):
        ...
        if message:
            ...
            await self.channel_layer.group_send(
                self.conversation_group_name,
                {"type": "message.update", "message": serialized_message},
            )
    ...
    async def message_receive(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "message.receive", "message": event["message"]}
            )
        )
    ...
    async def message_update(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "message.update", "message": event["message"]}
            )
        )
    ...
    async def message_delete(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "message.delete", "message_id": event["message_id"]}
            )
        )
    ...
    def update_presence(self, status):
        ...
        Presence.objects.update_or_create(
            user=self.user, defaults={"status": status, "last_seen": timezone.now()}
        )
    ...
    def edit_message(self, message_id, new_text):
        ...
        try:
            message = Message.objects.get(
                id=message_id, sender=self.user  # Só o remetente pode editar
            )
...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.channel_layer.group_add(
            self.user_notification_group_name, self.channel_name
        )
        await self.accept()
