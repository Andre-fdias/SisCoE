...
class Conversation(models.Model):
    ...
    name = models.CharField(
        max_length=128, blank=True, null=True, help_text="Nome do grupo, se aplicável"
    )
...
class Participant(models.Model):
    ...
    def __str__(self):
        return (
            f"{(self.user.get_full_name() or self.user.email)} in {self.conversation}"
        )
...
class Message(models.Model):
    ...
    quoted_text = models.TextField(
        blank=True, null=True, help_text="Snapshot do texto da mensagem respondida"
    )
...
class Attachment(models.Model):
    ...
    thumbnail = models.ImageField(
        upload_to="chat_thumbnails/%Y/%m/%d/", blank=True, null=True
    )
...
class MessageStatus(models.Model):
    ...
    participant = models.ForeignKey(
        Participant, on_delete=models.CASCADE, related_name="message_statuses"
    )
