...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("=== LIMPEZA DE MENSAGENS ANTIGAS ===")
        ...
        if dry_run:
            ...
            if count > 0:
                sample_messages = old_messages.select_related("sender", "conversation")[
                    :5
                ]
                self.stdout.write("\nAmostra de mensagens que seriam excluídas:")
                for msg in sample_messages:
                    preview = (
                        msg.decrypted_text[:50] + "..."
                        if msg.decrypted_text and len(msg.decrypted_text) > 50
                        else (msg.decrypted_text or "[Sem texto]")
                    )
            ...
            if count > 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ {deleted_count} mensagens excluídas com sucesso!"
                    )
                )
            else:
                self.stdout.write(
