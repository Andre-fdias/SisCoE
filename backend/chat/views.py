...
class ChatView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["all_users"] = User.objects.exclude(pk=self.request.user.pk).order_by(
            "email"
        )
...
class UserProfileAPIView(APIView):
    ...
    def get(self, request, user_id, *args, **kwargs):
        try:
            user = (
                User.objects.select_related("cadastro")
                .prefetch_related(
                    "cadastro__promocoes",
...
class UserPresenceView(APIView):
    ...
    def post(self, request):
        ...
        presence, created = Presence.objects.update_or_create(
            user=request.user,
            defaults={"status": status_val, "last_seen": timezone.now()},
        )
...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def _find_existing_1on1_conversation(self, participants_ids):
        ...
        conversations = (
            Conversation.objects.filter(
                is_group=False, participants__user_id__in=participants_ids
            )
            ...
            .annotate(
                match_count=Count(
                    "participants",
                    filter=Q(participants__user_id__in=participants_ids),
                    distinct=True,
                )
            )
    ...
    def create_or_open_private_conversation(self, request):
        ...
        if not target_user_id:
            return Response(
                {"error": "user_id é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class MessageViewSet(viewsets.ModelViewSet):
        ...
        try:
            ...
            if not Participant.objects.filter(
                ...
                raise serializers.ValidationError(
                    "O usuário não é participante desta conversa."
                )
    ...
    def react(self, request, conversation_pk=None, pk=None):
        ...
        if not emoji:
            return Response(
                {"error": "Emoji é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def perform_create(self, serializer):
        ...
        if message_id:
            try:
                ...
                if not Participant.objects.filter(
                    conversation=message.conversation, user=self.request.user
                ).exists():
                    raise serializers.ValidationError(
                        "Você não tem permissão para anexar arquivos nesta conversa."
                    )
...
class GroupViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        return (
            Conversation.objects.filter(participants__user=user, is_group=True)
            .prefetch_related(
                Prefetch(
                    "participants",
                    queryset=Participant.objects.select_related("user"),
    ...
    def add_member(self, request, pk=None):
        ...
        Participant.objects.get_or_create(
            conversation=conversation, user=user, defaults={"is_admin": False}
        )
...
class ChatStatisticsView(APIView):
    ...
    def get(self, request):
        ...
        unread_messages_count = MessageStatus.objects.filter(
            participant__user=user, status="delivered"
        ).count()
        ...
        active_conversations_data = [
            {
                "id": conv.id,
                "name": conv.name
                or f"Conversa com {conv.participants.exclude(user=user).first().user.display_name if conv.participants.exclude(user=user).exists() else 'Usuário'}",
...
class AdminChatView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"error": "Ação não reconhecida"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class SearchView(APIView):
    ...
    def get(self, request):
        ...
        messages = (
            Message.objects.filter(
                conversation__participants__user=user, text__icontains=search_term
            )
