    ...
    SEARCHABLE_MODELS = {
        ...
        "user": {
            ...
            "fields": [
                ...
                "first_name",
                "last_name",
            ],  # Removido campos que não existem
        ...
        "useractionlog": {
            ...
            "url": lambda obj: reverse(
                "accounts:user_action_history", args=[obj.user.pk]
            ),  # Link para o histórico de ações do usuário
        },
        # Adicional
        "cadastro_adicional": {
            "app": "adicional",
        ...
        "historicocadastro": {
            ...
            "url": lambda obj: reverse(
                "adicional:historico_adicional", args=[obj.cadastro_adicional.id]
            ),
        },
        # Agenda
        "lembrete": {
        ...
        "arquivo": {
            ...
            "fields": ["tipo"],
            "url": lambda obj: reverse(
                "documentos:documento_detail", args=[obj.documento.pk]
            ),
        },
        # Efetivo
        "cadastro": {
        ...
        "detalhes_situacao": {
            ...
            "url": lambda obj: reverse(
                "efetivo:historico_movimentacoes", args=[obj.cadastro.id]
            ),
        },
        "promocao": {
            "app": "efetivo",
            "model": "Promocao",
            "fields": ["posto_grad", "quadro", "grupo", "cadastro__re"],
            "url": lambda obj: reverse(
                "efetivo:historico_movimentacoes", args=[obj.cadastro.id]
            ),
        },
        ...
        "historicocatefetivo": {
            ...
            "fields": ["tipo", "observacao", "cat_efetivo__cadastro__re"],
            "url": lambda obj: reverse(
                "efetivo:historico_categorias", args=[obj.cat_efetivo.cadastro.id]
            ),
        },
        # LP
        "lp": {
        ...
        "pessoal": {
            ...
            "url": lambda obj: reverse(
                "municipios:editar_pessoal", args=[obj.posto.pk]
            ),
        },
        "cidade": {
            "app": "municipios",
    ...
    def search(cls, query):
        ...
        for key, config in cls.SEARCHABLE_MODELS.items():
            try:
                ...
                print(
                    f"Erro: Modelo '{config['model']}' no app '{config['app']}' não encontrado. Verifique INSTALLED_APPS e o nome do modelo."
                )
                continue
            ...
            if config["fields"]:
                for field in config["fields"]:
                    if (
                        ...
                        or model._meta.get_field(field).get_internal_type()
                        not in [
                            "IntegerField",
                        ...
                        try:
                            ...
                            print(
                                f"Aviso: Não foi possível criar a cláusula de busca para o campo '{field}' no modelo '{config['model']}' do app '{config['app']}'. Erro: {e}"
                            )
            ...
            if q_objects:
                try:
                    ...
                    print(
                        f"Erro ao buscar em {config['model']} (app: {config['app']}): {str(e)}"
                    )
    ...
    def format_result(cls, config, obj):
        ...
        if hasattr(obj, "get_search_result"):
            try:
                ...
                print(
                    f"Aviso: Erro ao chamar get_search_result() para {obj} ({config['model']}). Usando fallback. Erro: {e}"
                )
