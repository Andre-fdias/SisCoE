...
from django.db.models import Sum, Count  # Adicionado Sum e Count aqui
from backend.agenda.models import Lembrete, Tarefa
from django.views.generic import TemplateView
from backend.documentos.models import Arquivo
...
def index(request):
    ...
    if not cadastro_do_usuario:
        messages.warning(
            request,
            "Seu usuário não está vinculado a um cadastro militar. Algumas funcionalidades podem estar limitadas. Por favor, entre em contato com o administrador.",
        )
    ...
    def get_ocupante_por_funcao(funcao_nome):
        try:
            ...
            if ocupante:
                ocupante.latest_promocao = ocupante.promocoes.order_by(
                    "-data_alteracao"
                ).first()
            return ocupante
    ...
    imagens_carrossel = (
        Arquivo.objects.filter(tipo="IMAGEM")
        .select_related("documento")
        .order_by("-documento__data_documento")
    )
    ...
    if user_permissoes == "sgb" and cadastro_do_usuario:
        user_sgb = None
        latest_situacao = (
            DetalhesSituacao.objects.filter(cadastro=cadastro_do_usuario)
        ...
        if user_sgb:
            aniversariantes = aniversariantes_qs.filter(
                detalhes_situacao__sgb=user_sgb
            ).distinct()
        else:
            aniversariantes = (
                Cadastro.objects.none()
            )  # Sem SGB associado para usuário 'sgb', não mostra nenhum
    ...
    aniversariantes = aniversariantes.order_by("nasc__day").prefetch_related(
        "imagens", "promocoes", "detalhes_situacao"
    )[:100]
    ...
    for funcionario in aniversariantes:
        try:
            ...
            if hasattr(funcionario, "promocoes") and funcionario.promocoes.exists():
                funcionario.posto_grad_recente = (
                    funcionario.promocoes.order_by("-data_alteracao").first().posto_grad
                )
            else:
                funcionario.posto_grad_recente = None

            # Buscar o DetalhesSituacao mais recente para o SGB e Posto/Seção
            latest_situacao = funcionario.detalhes_situacao.order_by(
                "-data_alteracao"
            ).first()
            if latest_situacao:
            ...
            logger.error(
                f"Erro ao obter promoção ou detalhes de situação para {funcionario.email}: {str(e)}"
            )
            funcionario.posto_grad_recente = None
    ...
    user_action_logs = (
        request.user.useractionlog_set.all().order_by("-timestamp")[:10]
        if request.user.is_authenticated
        else []
    )
    ...
    lembretes = (
        Lembrete.objects.filter(
            user=request.user, data__year=hoje.year, data__month=hoje.month
        ).order_by("data")
        if request.user.is_authenticated
        else []
    )
...
def dashboard_view(request):
    try:
        ...
        efetivo_fixado = sum(
            v if v is not None else 0 for v in pessoal_aggregation.values()
        )
        ...
        percentual_claro_cb = (
            round((claro / efetivo_fixado * 100), 1) if efetivo_fixado > 0 else 0
        )

        bombeiros_municipais = Cadastro_bm.objects.filter(situacao="Efetivo").count()

        percentual_claro_cb_bcm = (
            round(
                (
                    (efetivo_fixado - (efetivo_existente + bombeiros_municipais))
                    / efetivo_fixado
                    * 100
                ),
                1,
        ...
        bm_mes_passado = Cadastro_bm.objects.filter(
            situacao="Efetivo", create_at__date__lte=mes_passado
        ).count()
        variacao_bm = calcular_variacao(bm_mes_passado, bombeiros_municipais)

        # Correção: usar 'detalhes_situacao' (com underscore)
        sgb_distribution = (
            DetalhesSituacao.objects.filter(situacao="Efetivo")
            .values("sgb")
            .annotate(
                existente=Count("id"),
            )
        ...
        for sgb in sgb_distribution:
            sgb_fixado = (
                Pessoal.objects.filter(posto__sgb=sgb["sgb"]).aggregate(
                    total=Sum("cel")
                    + Sum("ten_cel")
                    + Sum("maj")
                    + Sum("cap")
            ...
            sgb["percentage"] = (
                round((sgb["existente"] / sgb_fixado * 100), 1) if sgb_fixado > 0 else 0
            )
        ...
        for mov in recent_movements:
            movement_type = (
                "transfer"
                if "transfer" in mov.situacao.lower()
                else "admission" if "admission" in mov.situacao.lower() else "other"
            )
...
class CalendarioView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        eventos_gb = [
            {
                "title": "Aniversário EB Itaí",
                "start": f"{current_year}-02-23",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário do CB/SP",
                "start": f"{current_year}-03-10",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário COBOM Scb",
                "start": f"{current_year}-03-31",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário - PB Botucatu",
                "start": f"{current_year}-04-14",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Anivers. PB Itapeva",
                "start": f"{current_year}-05-29",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Feriado Nacional - Dia do Trabalho",
                "start": f"{current_year}-05-01",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Aniversário do CB/Bra",
                "start": f"{current_year}-07-02",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário PB Salto",
                "start": f"{current_year}-07-16",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário PB Tatuí",
                "start": f"{current_year}-08-08",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário BB Boituva",
                "start": f"{current_year}-09-03",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário PB Itu",
                "start": f"{current_year}-09-06",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário - PB Avaré",
                "start": f"{current_year}-09-15",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário BB Porto Feliz",
                "start": f"{current_year}-10-26",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário BB Tietê",
                "start": f"{current_year}-10-27",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário PB Cerrado",
                "start": f"{current_year}-11-12",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário PB Votorantim",
                "start": f"{current_year}-12-08",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário BB Apiaí",
                "start": f"{current_year}-12-10",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário da PM",
                "start": f"{current_year}-12-15",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário PB Eden",
                "start": f"{current_year}-12-16",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
            {
                "title": "Aniversário BB Cap.Bonito",
                "start": f"{current_year}-12-17",
                "color": "#3b82f6",
                "tipo": "Evento GB",
            },
        ]
        ...
        feriados_nacionais = [
            {
                "title": "Feriado Nacional - Ano Novo",
                "start": f"{current_year}-01-01",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Carnaval",
                "start": f"{current_year}-03-03",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Carnaval",
                "start": f"{current_year}-03-04",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Carnaval",
                "start": f"{current_year}-03-05",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Sexta-Feira Santa",
                "start": f"{current_year}-04-18",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Dia de Tiradentes",
                "start": f"{current_year}-04-21",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
            ...
            {
                "title": "Feriado Nacional - Corpus Christi",
                "start": f"{current_year}-06-19",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Independência do Brasil",
                "start": f"{current_year}-09-07",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Nossa Senhora Aparecida",
                "start": f"{current_year}-10-12",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Dia das Crianças",
                "start": f"{current_year}-10-12",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Dia do Professor",
                "start": f"{current_year}-10-15",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Dia do Servidor Público",
                "start": f"{current_year}-10-28",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Dia de Finados",
                "start": f"{current_year}-11-02",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
            ...
            {
                "title": "Feriado Nacional - Consciência Negra",
                "start": f"{current_year}-11-20",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
            {
                "title": "Feriado Nacional - Natal",
                "start": f"{current_year}-12-25",
                "color": "#ef4444",
                "tipo": "Nacional",
            },
        ]
        ...
        feriados_estaduais = [
            {
                "title": "Feriado Estadual - Revolução Constitucionalista",
                "start": f"{current_year}-07-09",
                "color": "#f59e0b",
                "tipo": "Estadual",
            }
        ]
        ...
        feriados_municipais = [
            {
                "title": "Feriado Municipal - Aniversário de Sorocaba",
                "start": f"{current_year}-06-15",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Dia do Padroeiro (São Pedro)",
                "start": f"{current_year}-06-29",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Votorantim",
                "start": f"{current_year}-08-25",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Piedade",
                "start": f"{current_year}-07-02",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Itu",
                "start": f"{current_year}-02-02",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Porto Feliz",
                "start": f"{current_year}-09-10",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Salto",
                "start": f"{current_year}-08-10",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de São Roque",
                "start": f"{current_year}-08-16",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Ibiúna",
                "start": f"{current_year}-03-24",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Itapeva",
                "start": f"{current_year}-09-20",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Itararé",
                "start": f"{current_year}-05-20",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Apiaí",
                "start": f"{current_year}-03-18",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Capão Bonito",
                "start": f"{current_year}-04-19",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Itapetininga",
                "start": f"{current_year}-11-05",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Angatuba",
                "start": f"{current_year}-12-20",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Boituva",
                "start": f"{current_year}-12-23",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Tatuí",
                "start": f"{current_year}-08-11",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Tietê",
                "start": f"{current_year}-03-21",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Laranjal Paulista",
                "start": f"{current_year}-10-27",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Botucatu",
                "start": f"{current_year}-04-14",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Itaí",
                "start": f"{current_year}-08-26",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Avaré",
                "start": f"{current_year}-09-15",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Itatinga",
                "start": f"{current_year}-12-20",
                "color": "#10b981",
                "tipo": "Municipal",
            },
            {
                "title": "Feriado Municipal - Aniversário de Piraju",
                "start": f"{current_year}-03-21",
                "color": "#10b981",
                "tipo": "Municipal",
            },
        ...
        todos_eventos = (
            eventos_gb + feriados_nacionais + feriados_estaduais + feriados_municipais
        )
