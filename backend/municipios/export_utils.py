...
def export_rpt_data(request, export_format="xlsx", **filters):
    ...
    try:
        ...
        logger.warning(
            "Modelo Cadastro_rpt não encontrado. A exportação RPT pode não funcionar."
        )
    ...
    if Cadastro_rpt:
        ...
        if filters.get("posto_secao_destino"):
            queryset = queryset.filter(
                posto_secao_destino=filters["posto_secao_destino"]
            )
        ...
        logger.info(
            "Não foi possível carregar Cadastro_rpt, retornando queryset vazio para RPT."
        )
...
def export_to_pdf_rpt(request, data):
    ...
    for sgb_display, postos in organized_data.items():
        sgb_elements = []
        sgb_elements.append(
            Paragraph(sgb_display, styles["RPTSGB"])
        )  # Usando o estilo renomeado

        for posto_display, items in postos.items():
            posto_elements = []
            posto_elements.append(
                Paragraph(posto_display, styles["RPTPosto"])
            )  # Usando o estilo renomeado
            ...
            posto_elements.append(
                Paragraph(
                    f"Total {posto_display}: {count_posto}", styles["RPTTotalPosto"]
                )
            )  # Usando o estilo renomeado
...
# ==============================================
def export_efetivo_pdf_report(
    request, report_data, sgb_filter=None, posto_secao_filter=None
):
    """
    ...
    elements.append(
        Paragraph("Relatório de Efetivo por Posto/Seção", styles["CustomTitle"])
    )
    elements.append(
        Paragraph(
    ...
    if posto_secao_filter:
        filter_info.append(
            f"Posto/Seção: {POSTO_SECAO_MAP.get(posto_secao_filter, posto_secao_filter)}"
        )

    if filter_info:
        elements.append(
            Paragraph("Filtros: " + ", ".join(filter_info), styles["CustomSubtitle"])
        )
    ...
    for sgb_display, posto_secoes in grouped_data.items():
        elements.append(
            Paragraph(f"SGB: {sgb_display}", styles["CustomSectionHeaderSGB"])
        )
        ...
        for posto_secao_display, item in posto_secoes.items():
            ...
            try:
                ...
                logger.warning(
                    "Modelos DetalhesSituacao ou Promocao não encontrados. Efetivo Existente pode não ser calculado."
                )
            ...
            if DetalhesSituacao and Promocao:
                ...
                logger.info(
                    f"Não foi possível calcular efetivo existente para {posto_obj.posto_atendimento} devido a modelos ausentes."
                )
            ...
            elements.append(
                Paragraph(
                    f"Posto/Seção: {posto_secao_display} - {posto_obj.posto_atendimento}",
                    styles["CustomSectionHeaderPostoSecao"],
                )
            )
            elements.append(Spacer(1, 2 * mm))
            ...
            elements.append(
                Paragraph("Efetivo Fixado (QPO)", styles["Normal"])
            )  # Título alterado
            table_fixado_header = [
            ...
            table_fixado_data = [
                ...
                [
                    Paragraph(
                        str(pessoal_planejado.get("cel", 0)), styles["CustomTableData"]
                    ),  # Dados de Cel
                    Paragraph(
                        str(pessoal_planejado.get("ten_cel", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(pessoal_planejado.get("maj", 0)), styles["CustomTableData"]
                    ),
                    Paragraph(
                        str(pessoal_planejado.get("cap", 0)), styles["CustomTableData"]
                    ),
                    Paragraph(
                        str(pessoal_planejado.get("tenqo", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(pessoal_planejado.get("tenqa", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(pessoal_planejado.get("asp", 0)), styles["CustomTableData"]
                    ),  # Dados de Asp
                    Paragraph(
                        str(pessoal_planejado.get("st_sgt", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(pessoal_planejado.get("cb_sd", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(pessoal_planejado.get("total_planejado", 0)),
                        styles["CustomTableDataBold"],
                    ),
                ],
            ]
            ...
            table_fixado = Table(
                table_fixado_data, colWidths=col_widths_fixado, style=table_fixado_style
            )
            ...
            elements.append(
                Paragraph("Efetivo Existente", styles["Normal"])
            )  # Título alterado
            table_existente_header = (
                [  # Atualizado para refletir os grupos que você tem
                    Paragraph("Ten Cel", styles["CustomTableDataBold"]),
                    Paragraph("Maj", styles["CustomTableDataBold"]),
                    Paragraph("Cap", styles["CustomTableDataBold"]),
                    Paragraph("Ten QO", styles["CustomTableDataBold"]),
            ...
            table_existente_data = [
                ...
                [
                    Paragraph(
                        str(efetivo_existente_grupos.get("Tc", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(efetivo_existente_grupos.get("Maj", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(efetivo_existente_grupos.get("Cap", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(efetivo_existente_grupos.get("Ten", 0)),
                        styles["CustomTableData"],
                    ),
                    Paragraph(
                        str(efetivo_existente_grupos.get("Ten QAOPM", 0)),
                        styles["CustomTableData"],
                    ),  # Usar a chave correta
                    Paragraph(
                        str(efetivo_existente_grupos.get("St/Sgt", 0)),
                        styles["CustomTableData"],
                    ),  # Usar a chave correta
                    Paragraph(
                        str(efetivo_existente_grupos.get("Cb/Sd", 0)),
                        styles["CustomTableData"],
                    ),  # Usar a chave correta
                    Paragraph(
                        str(total_efetivo_existente), styles["CustomTableDataBold"]
                    ),
                ],
            ]
            ...
            table_existente = Table(
                table_existente_data,
                colWidths=col_widths_existente,
                style=table_existente_style,
            )
            ...
            total_diff = total_efetivo_existente - pessoal_planejado.get(
                "total_planejado", 0
            )
            if pessoal_planejado.get("total_planejado", 0) == 0:
                ...
                total_percentage = (
                    total_diff / pessoal_planejado.get("total_planejado", 0)
                ) * 100
            ...
            diff_row.append(
                Paragraph(f"{total_diff} ({total_percentage:.2f}%)", style_total_diff)
            )

            table_diff_data = [table_diff_header, diff_row]
            ...
            table_diff = Table(
                table_diff_data, colWidths=col_widths_existente, style=table_diff_style
            )  # Usar col_widths_existente
            elements.append(table_diff)
            elements.append(Spacer(1, 8 * mm))

            if (
                posto_secao_display != list(posto_secoes.keys())[-1]
                or sgb_display != list(grouped_data.keys())[-1]
            ):
    ...
    return (
        pdf_buffer,
        "relatorio_efetivo.pdf",
    )  # Nome do arquivo para o relatório de efetivo
...
def export_to_csv_rpt(request, data):
    ...
    wrapper = io.TextIOWrapper(
        buffer, encoding="utf-8-sig", newline="", write_through=True
    )
