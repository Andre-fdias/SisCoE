...
)  # Importar ImportExportModelAdmin
...
class PostoResource(resources.ModelResource):
    ...
    class Meta:
        ...
        exclude = (
            "quartel",
        )  # Excluindo o campo de imagem para importação/exportação direta
...
class ContatoResource(resources.ModelResource):
    ...
    posto = fields.Field(
        ...
        ),  # Vincula pelo campo 'posto_atendimento' do modelo Posto
    )
...
class CidadeResource(resources.ModelResource):
    ...
    class Meta:
        model = Cidade
        fields = ("posto", "descricao", "municipio", "longitude", "latitude")
        export_order = fields
...
class PostoAdmin(ImportExportModelAdmin):  # Herda de ImportExportModelAdmin
    ...
    def quartel_preview(self, obj):
        if obj.quartel:
            return format_html(
                '<img src="{}" style="max-height: 200px;"/>', obj.quartel.url
            )
    ...
    fieldsets = (
        ...
        ("Localização", {"fields": ("cidade_posto", "tipo_cidade", "op_adm")}),
        (
            "Registro",
            {
                "classes": ("collapse",),
