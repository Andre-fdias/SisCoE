...
class AgendaViewsTest(TestCase):
    ...
    def test_criar_tarefa_view(self):
        ...
        data = {
            ...
            "data_fim": (timezone.now() + timedelta(days=1)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "cor": "#00FF00",
        }
        ...
        response = self.client.post(
            reverse("agenda:tarefa_nova"), data, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_protecao_usuario_lembrete(self):
        ...
        outro_user = User.objects.create_user(
            email="outro@example.com", password="outropass123"
        )
