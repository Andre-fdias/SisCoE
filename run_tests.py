...
def run_tests(apps=None, verbosity=2, failfast=False, coverage=False):
    ...
    if coverage:
        ...
        cov = coverage.coverage(
            source=['backend'],
            omit=['*/migrations/*', '*/tests/*', '*/__pycache__/*', '*/static/*', '*/templates/*']
        )
        cov.start()
...
def main():
    ...
    parser.add_argument('--verbosity', type=int, default=2, help='Nível de verbosidade (0-3)')
    ...
    if args.list:
        apps_list = [
            'accounts', 'bm', 'core', 'efetivo', 'municipios', 'rpt',
            'adicional', 'agenda', 'calculadora', 'crm', 'cursos', 
            'documentos', 'lp'
        ]
